<script lang="ts">
  ...
  /** Що робити, якщо позиції не буде на полиці. */
  export type SwapPolicy = 'substitute' | 'skip' | 'call'

  interface Props {
    lines: CartLine[]
    /** Рішення гостя по кожній позиції. Порожньо — діє «замінити за списком». */
    policy: Record<string, SwapPolicy>
    /** Ланцюжок, переписаний гостем: порядок, додане й прибране разом.
     *  Немає ключа — діє той, що склав агент. */
    chains: Record<string, Substitute[]>
    /** Авто-заміна (#12): ризикова позиція без ланцюжка їде з мандатом
     *  «рівноцінна заміна того самого виду» в ціновій вилці — вісь називає
     *  агент для кожної позиції окремо (#42), — а не питає погодження. */
    autoSwap: boolean
    onAutoSwap: () => void
    onPolicy: (id: string, value: SwapPolicy) => void
    onChain: (lineId: string, chain: Substitute[]) => void
    /** Перелік виду на слот прогону. Рахує сервер: слот і філія — з плану. */
    onOptions: (lineId: string, query: string) => Promise<Loaded<SwapOption[]>>
    /** Галочки замін «Сільпо» на цьому замовленні. Тільки читаємо (#49):
     *  писати в чуже поле означає зробити рівно те, чого продукт обіцяє
     *  не робити, а пульт керування їхніми налаштуваннями при реальній
     *  інтеграції однаково викинуть. */
    feedback: OrderFeedback
    /** Кошик зібрано за іншими умовами, ніж стоять зараз, — той самий
     *  підпис прогону, що й на екрані кошика. Правка ланцюжка входить у
     *  нього: мандат пише сервер, і поки кошик не перезібрано, в акаунт
     *  поїде текст зі старого плану. */
    stale: boolean
    /** Що саме розійшлось — словами, а не «ланцюжок змінився» завжди (#86). */
    staleWhy: string[]
    /** Перезібрати кошик під нові ланцюжки. */
    onRebuild: () => void
    /**
     * Запам'ятати ці ланцюжки для ЦИХ ВИДІВ надалі (#224).
     *
     * Явна галочка, а не виведене зі «щось погодив»: збережене мовчки -- це
     * налаштування, якого гість не ставив, і побачив би він його аж тоді,
     * коли воно спрацювало б не так, як він очікує.
     */
    remember: boolean
    onRemember: () => void
    /** Що вже погоджено надалі. Прибрати можна лише те, що ВИДНО (#126). */
    saved: SavedSwap[]
    onForget: (id: string) => void
    savedBusy: boolean
    /** Гість прийшов сюди з «Оформити», і звідси ж закінчує (#49). */
    confirming: boolean
    onConfirm: () => void
    onBack: () => void
    /**
     * Куди відкрити екран (D7): два входи з кошика ведуть сюди, і кожен --
     * у своє місце. `'urgent'` гортає до термінового; `'all'` (і
     * відсутність значення) лишає зверху -- це і є «переглянути всі».
     * Необов'язковий: доки App.svelte не прокидає його з `onSwaps` у кошику,
     * екран поводиться як досі.
     */
    focus?: 'urgent' | 'all'
  }
  ...
  /**
   * Що станеться з рядком БЕЗ мандата — словами і чесно (#106).
   *
   * Відповідь дає не наш екран, а галочка замовлення, яку ми вже читаємо
   * (`core/feedback.py`): «замініть на схожі» означає, що збирач підбере
   * сам, а `disapprovedChanges` — що не підбере. Третій стан («не звіряли»)
   * лишається третім: вигадувати за нього означало б повернути ту саму
   * поломку з іншого боку.
   */

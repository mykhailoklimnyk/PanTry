<script lang="ts">
  ...
  interface Props {
    basket: Basket
    /**
     * Приглушені гостем — «ще є вдома». Не фільтр: рядок лишається видимим,
     * а в live його стан приходить із сервера вже перерахованим, і множина
     * тут порожня.
     */
    once: Set<string>
    /** Рядки «ще є вдома», які гість усе-таки взяв. */
    /** Правки кількості: externalProductId → скільки брати. */
    qty: Record<string, number>
    /** Рядки, додані гостем поверх прогону: докинуте до порога і взяте з бару. */
    extras: CartLine[]
    mode: string
    rules: Exclusion[]
    /** Що не вдалося з правилами. Правити їх можна і звідси — отже й читати
     *  про відмову теж: примітка лише на старті невидима саме тоді, коли
     *  гість правив правило тут (#45). */
    rulesNote: string | null
    deliveryOptions: DeliveryOption[]
    /**
     * Умови доставки ЯК ВІДПОВІДЬ, а не як список (#185). `null` — запит ще
     * їде, `{ok:false}` — упав. До 02.09 обидва стани зводились у `[]`, і
     * кошик мовчав про мінімум і межу ваги так само, як при законному
     * порожньому переліку: «Оформити» не гасло, попередження не було.
     *
     * Формально це не регресія (зі старим скелетом числа теж були `null`),
     * але після #164 старт ОБІЦЯЄ це вголос — «кошик збереться, а доставку
     * не перевірить», — і обіцянку треба виконати саме там, де вона кусається.
     */
    deliveryLoaded: Loaded<DeliveryOption[]> | null
    /** Прочитати умови ще раз — той самий повтор, що на старті (#164). */
    onDeliveryRetry: () => void
    delivery: string
    /** Куди веземо і хто збирає — читається на старті, тут лише показується.
     *  Перед «Оформити» це остання можливість помітити чужу адресу. */
    place: Where | null
    budget: number
    /** Ціль і пресети з ЙОГО замовлень (#212). `null` — історію не прочитали. */
    spend: SpendTarget | null
    /** З чого порахувалась комора: чип суми каже правду про добір (#206). */
    pantrySource: Omit<Pantry, 'items'> | null
    draft: string
    /** Кошик зібрано за іншими налаштуваннями, ніж стоять зараз. */
    stale: boolean
    /** Підпис чипса: чим саме розійшлось (`lib/stale.ts`). */
    staleLabel: string
    touched: boolean
    /** Звіт про запис у акаунт «Сільпо». null — ще не оформлювали. */
    checkout: CheckoutResult | null
    /**
     * Питання про рядки, які вже лежать у кошику «Сільпо» (#30).
     *
     * Не звіт: поки воно стоїть, у кошик не пішло нічого. «Оформити» ДОДАЄ
     * рядки, тож залишене минулим прогоном тихо входило б у суму.
     */
    ...
    /** Відкрити вікно розбіжності — рішення ухвалює гість, не дотик. */
    onStale: () => void
    /** Відповідь на уточнення. Тягне ПОВНУ перезбірку: умова змінилась. */
    onAnswer: (answer: ClarifyAnswer) => void
    /** Гість обрав ТОВАР у відповідь: додається без перезбірки (#190). */
    onPick: (intent: string, pick: ClarifyPick) => void
    onCheckout: () => void
    /** Перезібрати кошик під сьогоднішню полицю. Стоїть лише там, де це
     *  справді щось міняє (#251). */
    onRebuild: () => void
    /** Лишити названу межу: слово гостя старше за рішення агента (10.09). */
    onKeepBudget: (named: number) => void
    /** Відповідь на питання про чужі рядки: лишити чи зняти перелічене. */
    onCarryOver: (existing: CarryOverState) => void
    /** Відкрити вибір адреси. Наслідки зважує App: кошик доведеться перезібрати. */
    onPlace: () => void
    /**
     * Відкрити погодження замін. `focus` каже, у яке місце екрана йти
     * (D7): 'urgent' -- гортати до термінового, 'all' -- лишитись зверху
     * («переглянути всі»). Приймає екран, а не цей файл: поки App.svelte не
     * прокидає значення в `<Swaps focus={...} />`, аргумент летить у
     * порожнечу без шкоди (див. HANDOFF-B.md).
     */
    onSwaps: (focus?: 'urgent' | 'all') => void
    /** Гість узяв дешевшу ланку того ж виду (#230). */
    onCheaper: (article: string, to: string) => void
    onTrace: () => void
    /** Показати стан замовлення. Кнопка є лише ПІСЛЯ запису: до нього
     *  замовлення не існує, і питати про нього нема про що (#50). */
    /** У картку ЦЬОГО рядка на екрані трейсу, а не «кудись у трейс» (#72). */
    onWhy: (id: string) => void
    /** Добір іде прямо зараз: кнопка мусить це показати, а не мовчати. */
    refilling: boolean
    /** Відповіді на уточнення, які ще в дорозі: намір → що обрано (#281). */
    answering?: Record<string, string>
    /** Наміри, які агент перепитав після відповіді (#281). */
    reasked?: string[]
    /** Добрати в цей самий кошик. Порожній перелік — «закрити решту тижня». */
  ...
  let extra = $state('')
  /** Що саме поїхало в добір — щоб слово гостя не зникало з екрана (#252). */

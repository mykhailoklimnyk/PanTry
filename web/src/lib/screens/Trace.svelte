<script lang="ts">
  ...
  interface Props {
    basket: Basket
    /**
     * Докинуте гостем після збірки. Агент обирав і ці рядки теж — картка
     * обґрунтування в них є, і без них дотик по докинутому рядку вів би у
     * порожнечу: у кошику він видимий, а в трейсі його не існувало.
     */
    extras: CartLine[]
    /**
     * Артикул рядка, з якого сюди прийшли. `null` — прийшли кнопкою «Що
     * зробив агент», і виділяти нема чого.
     */
    focus: string | null
    logOpen: boolean
    /**
     * Панель дебагу ввімкнена. До 07.09 цього пропа тут не було ВЗАГАЛІ:
     * плашку шукали очима, а її ніколи не малювали -- і «не доїхало» від
     * «не існує» на екрані не відрізнити. Розклади, які гість не читає
     * (як порахована стеля, чому агент не взяв вид), їдуть в `args` кроку,
     * а показує їх саме цей прапорець: панель дебагу живе лише від 1240 px,
     * тобто на телефоні тих чисел не видно ніде.
     */
    debug?: boolean
    /**
     * Скільки коштував цей ВХІД -- усі прогони разом, комора теж (прохання
     * власника 08.09). Стоїть поруч із ціною ЦЬОГО прогону і не заміняє її:
     * одне число про роботу, яку гість щойно бачив, друге про гаманець за
     * весь вхід, і згорнуті в одне вони відповідали б на обидва питання
     * неправильно.
     *
     * `null` -- лічильник не відповів або прогонів ще не було. Нуль тут не
     * малюється взагалі: він був би числом без осі (#76).
     */
    toll?: Toll | null
    onToggleLog: () => void
    onBack: () => void

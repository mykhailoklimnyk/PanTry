<script lang="ts">
  ...
  interface Props {
    items: PantryItem[]
    /**
     * Скільки коштував цей ВХІД -- усі прогони разом, кошик теж. Стоїть у
     * «звідки це» поруч із ціною ОБЕРТУ і не заміняє її: оберт про роботу,
     * яку гість щойно бачив, вхід -- про гаманець. `null` -- лічильник
     * мовчить або прогонів ще не було, і тоді рядка немає взагалі (#76).
     */
    toll?: Toll | null
    /** Комора вже відповіла. Без цього порожній список і порожня відповідь
     *  — те саме, а це різні речі (#56). */
    loaded: boolean
    /**
     * Коли почалось читання комори -- або `null`, якщо воно не йде (#56).
     *
     * Потрібне ТУТ з 06.09: до #341 у комору заходили вже прочитаною, тож
     * німим цей екран не бував ніколи, і крок показував лише старт. Тепер
     * перший показ саме тут -- і мовчати він не має права рівно тому, чому
     * не має права мовчати старт.
     */
    pending?: number | null
    /** Чому комора не прочиталась. `null` -- прочиталась (#166). */
    failed: string | null
    onRetry: () => void
    /** З чого комора порахувалась: причина порожнечі, а не самі рядки (#38). */
    source: Omit<Pantry, 'items'> | null
    addOpen: boolean
    query: string
    onQuery: (value: string) => void
    /**
     * Рядки, чиє слово ще не доїхало до сервера (#145).
     *
     * Множина, а не один id: правки стоять у черзі й ідуть по одній, і
     * поки вони їдуть, СТАН цих рядків — наш старий висновок. Число в
     * степері вже нове (це слово гостя), а смуга й «ще ~N дн» рахуються
     * сервером з того числа — тож поруч вони показували б різні епохи.
     */
    busy: ReadonlySet<string>
    /**
     * «Вже купив»: подія, яка зсуває точку відліку циклу (#73).
     *
     * `qty` — скільки гість узяв, якщо він це назвав (#99). `null` означає
     * «просто купив»: кількості не було, і вигадувати її не можна.
     */
    onBought: (id: string, qty: number | null) => void
    onAddToggle: () => void
    /** Додати вид, якого не знайшлось у коморі: назва вже набрана в пошуку. */
    onAddNamed: (label: string) => void
    onAdd: () => void
    /** Прибрати рядок, доданий руками. Рядок з чеків сюди не приходить. */
    onForget: (id: string) => void
    /** «Не показуй цей вид» і зворотне до нього (#124). Ключ той самий, що
     *  веде решту слів гостя: у `hide` це id рядка, у `unhide` -- підпис,
     *  бо саме він стоїть на кнопці повернення. */
    onHide: (id: string) => void
    onUnhide: (label: string) => void
    /**
     * «Це не один вид» і зворотне до нього (#335, #19).
     *
     * Ключ тут НАМІР, а не id рядка: гість показав на ЗАГОЛОВОК, а заголовок
     * рядком не є -- він стоїть над кількома. Ним же ключується черга правок
     * нагорі: два різні наміри під спільним ключем перезаписали б одне
     * одного (#174).
     */
    onSplit: (intent: string) => void
    onUnsplit: (intent: string) => void
    /**
     * Мандат виду: погодити ланцюжок замін просто на рядку або зняти (#332).
     *
     * Порожній перелік означає «зняти», і це не скорочення: у явному рішенні
     * гостя порожній ланцюжок означав би «не привозити взагалі» (#224), тож
     * розводить їх сервер двома різними словами, а не одним із порожнечею.
     *
     * Артикули, а не назви: рядок мандата збирає сервер з чеків гостя, бо
     * назва, надіслана звідси, стала б нашою обіцянкою від його імені про
     * товар, якого він не називав (#128).
     */
    onMandate: (id: string, chain: string[]) => void
    /**
     * «На наступну покупку»: вид їде в список (#338, #110).
     *
     * Аргумент -- ПІДПИС, а не id рядка: у список кладеться слово, і ключ з
     * нього рахує сервер тим самим `manual_key`, яким кладе набране руками.
     * Id рядка тут не годиться взагалі -- він у трьох різних формах
     * (артикул, відбиток дописаного, ключ виду), і список про них не знає.
     */
    onToList: (label: string) => void
    onAdjust: (id: string, delta: number) => void
    /**
     * «Вистачає на N днів» — слово гостя про ЧАСТОТУ (#144).
     *
     * `null` знімає назване: рядок повертається до оцінки з чеків, а не
     * порожніє. Окремо від `onBought` навмисно: то подія («поповнив»), це
     * властивість виду («треба раз на два дні»), і одна дія на обидва
     * мовчки стирала б одне слово другим.
     */
    onCycle: (id: string, days: number | null) => void
    /**
     * Чим наповнюється СПИСОК: покупками чи самим гостем (#109).
     *
     * Прапорець явний, а не виведений із «є дописані рядки»: виведений злив
     * би «я все видалив» і «я ще нічого не дописував» в один порожній екран,
     * а поради в них протилежні (#38).
     */
    onSource: (mode: 'receipts' | 'manual') => void
    /** Скласти список з покупок — явною дією з видимим результатом. */
    onGenerate: () => void
    /**
     * Скласти список НА НАСТУПНУ ПОКУПКУ з комори (#306).
     *
     * Це інша дія, ніж «скласти з покупок»: та переносить у комору види,
     * які гість бере, а ця бере з комори те, що ЗАКІНЧУЄТЬСЯ, і кладе в
     * список на потім (#110). Комора каже, коли вид закінчиться; список --
     * що взяти цього разу.
     */
    onNextList: () => void
    /** Стерти ВЕСЬ список, доданий руками. Рядків з чеків не чіпає (#124). */
    onWipe: () => void
    /** Що зробила остання дія — словами (#245). `null` — дії не було. */
    deed: string | null
    /** Складання списку ТРИВАЄ -- окремо від `sourceBusy` (див. кнопку). */
    composing: boolean
    /** Котрась із трьох дій зараз їде на сервер: вони довгі (перерахунок). */
    sourceBusy: boolean
    /**
     * Панель дебагу ввімкнена -- показати смужку стану (#330).
     *
     * Прохання власника: «як флаттер в хедері пише реліз-дебаг версія».
     * Смужка не для гостя і не для журі: вона каже, ЯК продукт зараз
     * працює (перечитав чи взяв з пам'яті, крутилась петля чи ні), а не
     * що в домі. У релізі її немає з тієї ж причини, з якої немає
     * флаттерівського банера: вона відповідає на питання, якого гість
     * не ставить.
     */
    debug?: boolean
    /**
     * Кроки ПЕТЛІ, поки вона йде (#341). Порожньо -- петля не крутиться або
     * ще не сказала першого слова.
     */
    steps?: TraceStep[]
    /** Петля саме зараз працює: шапка каже, що робиться, а не мовчить. */
    looping?: boolean
    asked?: PantryAsk[]
    /** Петля крутиться ПІСЛЯ відповіді гостя: екран лишається закритим. */
    answering?: boolean
    /** «Ще питання»: агент дивиться, що ще спитати, -- це не запис відповіді. */
    answeringMore?: boolean
    /** Питання, відкладені гостем: чекають за дверима зі своїм числом (#386). */
    door?: PantryAsk[]
    /** Відчинити двері: ті самі питання на тому самому екрані. */
    onDoor?: () => void
    onAnswer?: (answers: Record<string, number>, more?: boolean) => void
    /**
     * Коли ПОЧАЛОСЬ заповнення (`Date.now()`), а не коли з'явився цей блок.
     *
     * Окремо від `pending`, хоч починаються вони разом: `pending` гасне з
     * ПЕРШОЮ відповіддю, а вікно агента стоїть і далі -- поки петля називає
     * види. Лічильник, перезапущений на середині, показував би «3 с» на
     * сороковій секунді очікування (той самий клас, що з відліком від появи
     * блока в `PantryWait`).
     */
    since?: number | null
  ...
  /**
   * Чи є під рядком що розкривати (09.09).
   *
   * Питання не про «чи багато тексту», а про те, чи не бреше контрол: кнопка,
   * яка відкриває порожнечу, гірша за її відсутність (#132). Тому перелік тут
   * рівно той, що всередині.
   */
  ...
  /** Скільки питань уже має відповідь. Вісь підпису кнопки. */
  const answered = $derived(Object.keys(answers).length)

  /** Відповіли на всі -- далі це ВИБІР, а не підтвердження (#386). */
  const allAnswered = $derived(asked.length > 0 && answered === asked.length)

  /** Надіслати сказане; `more` -- «питай ще», а не «веди в комору». */
  ...
  /** Що гість попросив не показувати -- підписами, з відповіді сервера. */
  ...
  <div class="pantry" data-testid="pantry-screen" bind:this={pantryEl}>
    ...
    {#if loaded && source && source.trace.length > 0}
      <details class="why" data-tour="pantry-why">
        <summary>звідки це: {source.receipts} чеків, {source.orders} замовлень</summary>
        <ol>

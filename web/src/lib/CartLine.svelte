<script lang="ts">
  ...
  interface Props {
    line: CartLine
    /** «Разова покупка»: у комору не потрапляє і на цикли не впливає. */
    once: boolean
    onToggleOnce: () => void
    /** ✕ на звичайному рядку, «додати» на приглушеному. */
    onAction: () => void
    onQty: (next: number) => void
    /**
     * У картку «Чому саме ці позиції» на екрані трейсу. `null` — картки в
     * цього рядка немає (напій з бару назвав гість), і тоді назва лишається
     * текстом: посилання, яке нікуди не веде, гість читає не як «тут агент
     * нічого не вирішував», а як поламаний продукт (#72).
     */
    onWhy: (() => void) | null
    /** Скільки рядків кошика починаються з того ж слова; 1 -- один. */
    kin?: number
    /** Дотик по підказці «дешевше того ж виду». null -- рядок її не показує. */
    onCheaper?: (() => void) | null
  }

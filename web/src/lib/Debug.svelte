<script lang="ts">
  ...
  interface Props {
    open: boolean
    screen: Screen
    /** Id обраної моделі — те, що поїде в BuildRequest.model. */
    model: string
    models: Loaded<ModelOption[]>
    /** Поточний підпис наміру і підпис, з яким зібрано кошик. */
    signature: string
    builtWith: string
    stale: boolean
    basket: Basket | null
    /** Кроки збірки, яка ЙДЕ зараз (02.09): панель показує їх у мить появи,
       а не після кошика. Порожньо -- збірки немає, показуємо останній трейс. */
    live?: TraceStep[]
    /** Кроки ПЕТЛІ КОМОРИ (#341). Окремим пропом, а не домішані в `live`:
       канал у них один, але роботи ДВІ, і вони йдуть одночасно -- петля
       крутиться на відкритті, а гість тим часом може тиснути «Зібрати».
       Злиті в один список, вони розійшлись би мовчки (#158), а на екрані
       «під капотом» це виглядало б як один прогін з чужими кроками.

       Живий тест 07.09: «кроки з комори не йдуть у "під капотом"» -- сюди
       не їхало взагалі нічого, тобто найдовша робота продукту (називання
       133 видів, ~28 с) під панеллю дебагу не існувала. */
    pantry?: TraceStep[]
    /**
     * Скільки з'їв ЦЕЙ ЗАБІГ -- комора і замовлення РАЗОМ (прохання власника
     * 08.09: «в трейсі не бачу ні токенів, ні затрат... хоч загальну, в цьому
     * забігу, доки сторінку не оновив»).
     *
     * Гроші комори писались у `agent_runs` з 29.08 і не показувались ніде,
     * крім SQL, -- а комора це найдорожчий екран на холодному акаунті. Число
     * стоїть ЗВЕРХУ, а не під трейсом: по нього заходять окремо, і шукати
     * його під двадцятьма кроками означало б не показати його зовсім.
     *
     * `null` -- лічильник мовчить або від відкриття вкладки ще нічого не
     * витрачено. Нуль тут був би числом без осі.
     */
    spent?: Toll | null
    /** Те саме за весь ВХІД -- переживає F5 і рестарт API. Стоїть поруч, бо
     *  без нього «цей забіг» не має з чим порівнятись, а два числа під одним
     *  словом розійшлись би мовчки (#158). */
    spentLogin?: Toll | null
    /** Прогони цієї сесії по порядку. Після уточнення їх уже два, і цікава
     *  саме РІЗНИЦЯ між ними: що змінила відповідь гостя (#55). Правка
     *  рядка — теж прогін: у неї свій `runId` (#84). */
    runs: Run[]
    /** Скільки прогонів витіснила стеля списку. Число тут, а не в панелі,
     *  бо витісняє їх App: панель показує рівно те, що їй дали (#91). */
    hidden: number
    error: string | null
    /** Відповідь `/api/health`: звідти беремо коміт БЕКЕНДА. `null` — ще не
     *  питали або питати нема кого (статичний ярус без `/api`). */
  ...
  /** «+1 рядок · -1 питання · +129 ₴» — рівно те, що змінилось. */
  ...
  /** Останній запис, у якого є кошик. Невдалі спроби прозорі для різниці. */

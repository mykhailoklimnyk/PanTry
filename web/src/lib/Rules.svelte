<script lang="ts">
  ...
  interface Props {
    rules: Exclusion[]
    /**
     * Що не вдалося з правилами — приміткою біля них, а не банером.
     *
     * Живе В КОМПОНЕНТІ, а не в екрані: правила стоять на старті й у кошику,
     * і відмова, показана лише на одному з них, невидима саме тоді, коли
     * гість правив правило на другому.
     */
    note?: string | null
    /** Способи отримання. Порожньо на старті — там свій блок. */
    deliveryOptions?: DeliveryOption[]
    /** Той самий перелік як ВІДПОВІДЬ: «ще їде» і «впало» -- різні (#185). */
    deliveryLoaded?: Loaded<DeliveryOption[]> | null
    onDeliveryRetry?: () => void
    delivery?: string
    budget: number
    /** Ціль і пресети з замовлень гостя (#212). `null` — не прочиталось. */
    spend: SpendTarget | null
    /**
     * З чого порахувалась комора -- і чим взагалі можна добирати під суму
     * (#206). `null` — ще не відповіла, і тоді чип нічого не стверджує:
     * «не знаю» не привід ні обіцяти, ні відмовляти (#89).
     */
    pantrySource?: Omit<Pantry, 'items'> | null
    draft: string
    /** У кошику — горизонтальна стрічка; на старті — блок із переносами. */
    strip?: boolean
    /** Чипс режиму збірки показуємо тільки в кошику. */
    modeLabel?: string | null
    /** Кошик зібрано за іншими налаштуваннями, ніж стоять зараз. */
    stale?: boolean
    /** Чим саме розійшлось — рахує `lib/stale.ts`, а не цей компонент. */
    staleLabel?: string
  ...
  /** Диктофон для поля правила — той самий, що на старті й у коморі. */
  let dictOpen = $state(false)
  /** Відмова розпізнавання показується, не ковтається (живий випадок 14.08). */

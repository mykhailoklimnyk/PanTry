<script lang="ts">
  ...
  const noHouse = $derived(
    results !== null && results.length > 0 && results.every((option) => !option.confirmed),
  )
...
<Modal label="Куди веземо">
  ...
  {:else if results !== null}
    {#if noHouse}
      <p class="note">
        «Сільпо» не знайшло такого номера будинку — перевір номер або допиши його
      </p>
    {/if}
    <ul class="found">
      {#each results as option (option.label)}
        <li>
          ...
          >
            <span class="option-label">{option.label}</span>
            {#if !option.confirmed}<span class="tag">без номера будинку</span>{/if}
          </button>
        </li>
      {/each}
    </ul>

<script lang="ts">
  ...
  async function refinePantryState(
    ...
    const again = () =>
      loopPantry(progressKey, pantryCold, fast, answers, covers, kinds, seen, more)
    let done = await again()
    if (!done.ok && run === pantryRun && (done.status === null || done.status >= 499)) {
      done = await again()
    }
    stopWatching()
    refreshQuota()
    ...
    if (
      ...
      !done.ok &&
      pantry.length > 0 &&
      pantry.every((row) => !row.named)
    ) {
      pantryFailed = `Комору не вдалось догрузити: ${done.message}.`
    }

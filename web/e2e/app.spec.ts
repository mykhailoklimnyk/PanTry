...
test("обірвана петля холодної комори пробує вдруге і не мовчить", async ({ page }) => {
  let loops = 0;
  page.on("request", (request) => {
    if (request.url().includes("/api/pantry/refine")) loops += 1;
  });
  await openPantryApp(page, { pantryCold: true, pantryLoop: "dead" });

  await expect(page.getByText(/Комору не вдалось догрузити/)).toBeVisible({
    timeout: 15_000,
  });
  await expect(page.getByRole("button", { name: "Спробувати ще раз" })).toBeVisible();
  expect(loops, "перший запит і рівно один повтор").toBe(2);
});
...
test("замовлення без товарів не радять чекати на три покупки", async ({ page }) => {
  await mockApi(page, { pantry: "no-contents" });
  await skipIntro(page);
  await enter(page);

  await expect(page.getByText(/не показує, що в них було/)).toBeVisible();
  await expect(page.getByText(/Перші покупки вже бачу/)).toHaveCount(0);
});
...
test("дописане руками відчиняє збірку на тиждень (#105)", async ({ page }) => {
  await mockApi(page, { pantry: "empty" });
  await skipIntro(page);
  await enter(page);
  await page.getByRole("button", { name: "на тиждень", exact: true }).click();
  ...
  await page.getByPlaceholder(/вид, а не марка/).fill("васабі");
  await page.getByRole("button", { name: "Додати в комору" }).click();
  await expect(page.getByText(/у чеках «Сільпо» такого немає/)).toHaveCount(1);
...
test("рядок з бару прибирається на місці, а не походом на сервер (17.09)", async ({
  page,
}) => {
  await openApp(page);
  await toStart(page);
  ...
  const drop = page.getByRole("button", { name: /^Прибрати Nemiroff/ });
  await expect(drop).toBeVisible();
  await drop.click();
  await expect(drop).toBeHidden();
  await expect(page.getByText(/у цьому прогоні немає/)).toBeHidden();
});
...
test("сервісний збір самовивозу стоїть окремим рядком у сумах (MCP 1.111.1)", async ({
  page,
}) => {
  await openApp(page, { serviceFee: 9 });
  await buildWeek(page);
  await checkout(page);

  const report = page.locator(".paid");
  await expect(report.getByText("сервісний збір")).toBeVisible();
  await expect(report.getByText("сервісний збір").locator("xpath=..")).toContainText(/9\s?₴/);
});

test("без збору рядка збору немає: нуль не малюємо", async ({ page }) => {
  await openApp(page);
  await buildWeek(page);
  await checkout(page);

  await expect(page.locator(".paid").getByText("до оплати")).toBeVisible();
  await expect(page.locator(".paid").getByText("сервісний збір")).toHaveCount(0);
});
...
test("варіант адреси без номера будинку видно, але обрати не можна (MCP 1.111.1)", async ({
  page,
}) => {
  let chosen: Record<string, unknown> | null = null;
  page.on("request", (request) => {
    if (request.method() === "POST" && new URL(request.url()).pathname === "/api/place") {
      chosen = request.postDataJSON() as Record<string, unknown>;
    }
  });
  await openApp(page);
  await page.getByRole("button", { name: /змінити/ }).click();

  await page.getByLabel("Адреса доставки").fill("Вінниця, Пирогова, 999");
  await page.getByRole("button", { name: "Знайти" }).click();

  const street = page.getByRole("button", { name: /Пирогова без номера будинку/ });
  await expect(street).toBeDisabled();
  await expect(page.getByText(/не знайшло такого номера будинку/)).toHaveCount(0);

  await page.getByRole("button", { name: "Вінниця, вулиця Пирогова, 20" }).click();
  await expect(page.getByText("Вінниця, вулиця Пирогова, 20", { exact: true })).toBeVisible();
  expect(chosen).toMatchObject({ street: "вулиця Пирогова", house: "20" });
});

test("коли будинку не підтвердив жоден варіант, екран каже це про запит", async ({
  page,
}) => {
  await openApp(page, { found: FOUND.filter((option) => !option.confirmed) });
  await page.getByRole("button", { name: /змінити/ }).click();

  await page.getByLabel("Адреса доставки").fill("Вінниця, Пирогова, 999");
  await page.getByRole("button", { name: "Знайти" }).click();

  await expect(page.getByText(/не знайшло такого номера будинку/)).toBeVisible();
  await expect(page.getByRole("button", { name: /без номера будинку/ })).toBeDisabled();
});
...
test("докинуте нами і зняте нами ж видно поіменно, окремо від «не влізло»", async ({
  page,
}) => {
  await mockApi(page, {
    basket: {
      ...
      fillCut: [
        {
          intent: "Сік Садочок 1л",
          name: "Сік Садочок яблучний 1л",
          price: 41.5,
          reason: "докинув це я під ціль 300 грн — на справжній ціні не влізло",
        },
      ],
    },
  });
  ...
  const mine = page
    .locator(".missed")
    .filter({ hasText: "Докинув під ціль і зняв сам" });
  await expect(mine).toContainText("Сік Садочок яблучний 1л");
  await expect(mine).toContainText("на справжній ціні не влізло");

  const cut = page.locator(".missed").filter({ hasText: "Не влізло в 300" });
  await expect(cut).not.toContainText("Сік Садочок");
});
...
test("каталог Bedrock у меню моделей ходить за прапорцем дебагу з бургера, одразу і в обидва боки (10.09)", async ({
  ...
  await expect(
    menu.getByText(/видно, бо панель дебагу увімкнена/),
  ).toBeVisible();
  await page.keyboard.press("Escape");
...
test("самовивіз зі збором не підписаний «безкоштовно» (MCP 1.111.1)", async ({
  page,
}) => {
  await mockApi(page);
  await skipIntro(page);
  ...
  const section = page.locator('[data-tour="delivery"]');
  await section.getByRole("button", { name: "самовивіз" }).click();
  await expect(section).toContainText(/збір\s9\s?₴/);
  await expect(section.getByText(/безкоштовно/)).toHaveCount(0);
});
...
test("докинуте до порога їде в запис без перезбірки: «Оформити» не гасне, тіло везе extras (11.09)", async ({
  ...
  await page
    .locator(".eco")
    .getByRole("button", { name: /до порога/ })
    .click();
  await expect(button).toBeEnabled();
  await expect(page.getByText(/докинуте на екрані ще не в плані/)).toHaveCount(
    0,
...
test("без дебагу на екрані збору кроки видно словами, а імена інструментів -- ні (#288, 10.09)", async ({
  ...
  await expect(running.locator(".step.live")).toContainText(
    "чеків прочитано: 46",
  );
...
test("згорнутий близнюк називає себе поіменно, а не зникає мовчки (20.09)", async ({
  page,
}) => {
  await mockApi(page, {
    basket: {
  ...
  const note = page.getByTestId("twins-dropped");
  await expect(note).toContainText("Томат Есміра рожевий");
  await expect(note).toContainText("Томат");
  await expect(note).toContainText("одна потреба");
});

test("на кошику без пар одного виду банера про близнюків немає (20.09)", async ({
  page,
}) => {
  await mockApi(page);
  await skipIntro(page);
  ...
  await expect(page.getByTestId("twins-dropped")).toHaveCount(0);
});
...
test.describe("питання агента в коморі (#386)", () => {
  ...
  test("відповідь продовжує той самий оберт, а питання зникають", async ({
    ...
    await page
      ...
      .click();
    await expect(
      page.getByRole("button", { name: "Ще питання" }),
    ).toBeVisible();
  ...
  test("екран з питаннями не пастка: вихід один і він зберігає відповідь", async ({
    ...
    await expect(
      page.getByRole("button", { name: "відкласти всі" }),
    ).toHaveCount(0);
    await expect(page.locator(".cover .dock")).toHaveCount(0);
    await expect(page.getByTestId("pantry-asked-done")).toBeVisible();
  });
});
...
test("межа під привід -- рішення агента, і гість вертає своє число одним дотиком", async ({
  ...
  const sent = page.waitForRequest(
    (request) =>
      request.method() === "POST" &&
      (request.postData() ?? "").includes('"budgetSaid":true'),
  );
...
test("рядок під заголовком групи лишається сіткою: значок поруч з назвою (10.09)", async ({
  ...
  expect(name?.x ?? 0).toBeGreaterThan(
    (thumb?.x ?? 0) + (thumb?.width ?? 0) - 1,
  );
});
...
test("перемикач кешу не перемикає себе: перший вхід холодний, оновлення тепле (10.09)", async ({
  ...
  await expect(page.getByText("не читати")).toBeVisible();
  expect(
    await page.evaluate(() => localStorage.getItem("komora:cold")),
  ).toBeNull();
...
test("нуль викликів моделі пишеться словами, а не «0 викликів» (10.09)", async ({
  ...
  test.skip(
    testInfo.project.name !== "desktop",
    "бічна панель дебагу живе від 1240 px",
  );
  await page.addInitScript(() => localStorage.setItem("komora:debug", "1"));
  await openApp(page);
  await buildWeek(page);
  ...
  const step = page
    .locator("article.step", { hasText: "get_orders_history" })
    .first();
  await expect(step).toContainText("модель не питали");
  await expect(step).not.toContainText("0 викликів");
  const planned = page
    .locator("article.step", { hasText: "agent.plan" })
    .first();
  await expect(planned.locator(".spend")).toHaveCount(0);
});

test("наростаючий фінал Android дає приріст, а не повтор фрази (10.09)", async ({
  page,
}) => {
  await stubSpeech(page);
  await openPantryAdd(page);
...
test("док «До покупок» у вікні заповнення має ті самі відступи, що в готовій коморі", async ({
  ...
  await expect(screen.getByText("Заповнюю комору")).toBeVisible({
    timeout: 15_000,
  });
  ...
  expect(
    frame.x + frame.width - (button.x + button.width),
  ).toBeGreaterThanOrEqual(12);
});

test("годинник іде і тоді, коли записується відповідь гостя", async ({
  page,
}) => {
  ...
  await expect(window.locator(".clock")).toHaveText(/^[1-9]\d* с$/, {
    timeout: 3_000,
  });
});

test("«Ще питання» дає ЩЕ ПИТАННЯ, а не веде в комору (16.09)", async ({
  page,
}) => {
  await openPantryApp(page, { pantryLoop: "asking" });
  const asked = page.getByTestId("pantry-asked");
  await asked.waitFor({ timeout: 15_000 });
  await expect(asked).toContainText("хліб");

  await asked.getByRole("button", { name: "на 3 дні" }).click();
  const more = page.getByRole("button", { name: "Ще питання" });
  await expect(more).toBeVisible();

  const sent = page.waitForRequest(
    (request) =>
      request.method() === "POST" &&
      request.url().includes("/pantry/refine") &&
      (request.postData() ?? "").includes('"more":true'),
  );
  await more.click();
  await sent;

  await expect(asked).toBeVisible({ timeout: 15_000 });
  await expect(asked).toContainText("молоко");
});
...
test("рядок без погодженої заміни їде без мандата, і звіт називає його", async ({
  ...
  const waiting = LINES.filter(
    (line) => line.needsApproval && line.mandate === null,
  );
  expect(waiting.length).toBeGreaterThan(0);
  await expect(
    page.getByText(`Без мандата поїхало ${waiting.length} поз.`, {
      exact: false,
    }),
  ).toBeVisible();
  await expect(
...
test("картка питань: чипи відповіді головні, хрестик угорі, решта тихим рядком (11.09)", async ({
  ...
  const [closeBox, chipBox] = await Promise.all([
    close.boundingBox(),
    chips.first().boundingBox(),
  ]);
  expect(closeBox !== null && chipBox !== null && closeBox.y < chipBox.y).toBe(
    true,
  );
...
test("мікрофон стоїть усередині поля «Додати ще», а не збоку (11.09)", async ({
  page,
}) => {
  await stubSpeech(page);
  await openApp(page);
...
test("модель без ключа зачиняє вхід: ворота ключа на вході, а не в коморі (11.09)", async ({
  ...
  await page
    .locator(".models .menu")
    .getByRole("option", { name: /GPT-5.6 Luna/ })
    .click();
  await page.keyboard.press("Escape");

...
class Want:
    ...
    at_home: bool = True
    origin: str = "guest"
    """Хто поклав рядок: `guest` -- набрав сам, `agent` -- склала кнопка (#306).

    Без цього перерахунок списку або стирає слово гостя, або не чіпає
    нічого: у таблиці лежать однакові рядки з двох різних дій, і різниця в
    них не в тексті, а в тому, ХТО СКАЗАВ (той самий вибір, що з `origin`
    рядка комори, 0027)."""
    why: str | None = None
    """Підстава словами: «закінчився вчора», «візьму, якщо буде акція».

    `None` -- підстави немає, і це не порожній рядок: рядок гостя підстави
    не потребує взагалі, він і Є рішення (#110)."""

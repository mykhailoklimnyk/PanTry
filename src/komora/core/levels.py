...
class Level:
    ...
    id: str
    intent: str
    """Намір, яким рядок називається. Порожньо -- назви від агента немає, і
    групувати нема за чим: рядок без мітки ключується першими словами чека,
    тобто близнюком за побудовою не буває."""
    urgent: bool
    """Чи закінчується. Порядок груп стоїть на ньому, а не на розмірі: те, що
    закінчується, -- згори, і згортання не має права це поховати (#107)."""
...
class Folded:
    ...
    order: tuple[str, ...]
    """Id рядків у порядку показу. Члени групи стоять поруч."""
    group_of: Mapping[str, str]
    """Id рядка -> назва групи. Рядка тут немає, якщо він стоїть сам."""
    groups: int
    """Скільки заголовків з'явилось. Нуль -- близнюків у цій коморі немає."""
    rows: int
    """Скільки рядків під ними згорнуто. Разом з `groups` це і є те, що
    називає крок трейсу: «38 назв у 15 рядках» (#334)."""
...
class Seen:
    ...
    days_since: int | None
    article: str = ""
    """Артикул чека (`lagerId`). Порожньо -- дописаний руками рядок: чеків під
    ним немає за побудовою (#126).

    Тут, а не в `Part`: артикул НЕ їде на екран (він нічого не каже гостю), а
    потрібен ПОЛИЦІ -- карта каталогу ключується саме ним (#337). Поле лежить
    поруч із рештою того, що бачив чек, бо злиття видів забирає всі SKU, крім
    найсвіжішого, і після нього артикулів уже немає (#335)."""
...
class Part:
    ...
    days_since: int | None
    fresh: bool
    """Гість бере це ОСТАННІМ ЧАСОМ (`RECENT_DAYS`, #266). Не «найчастіше»:
    частка по всій історії тягне в «звичне» те, що куплено позаторік."""

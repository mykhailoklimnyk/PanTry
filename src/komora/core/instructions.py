...
class Verdict:
    ...
    verdict: str
    where: str = ""
    """Де це живе в нас: модуль, номер задачі або причина відмови."""
...
class Audit:
    ...
    judged: int = 0
    unjudged: tuple[Gap, ...] = ()
    """Нові або переписані абзаци: вироку немає, у промпт не їдуть."""
    orphaned: tuple[Gap, ...] = ()
    """Вироки без абзацу: інструкцію переписали або зняли, і наш код може
    виконувати вимогу, якої вже немає."""
    broken: tuple[Gap, ...] = ()
    """Криві рядки реєстру: невідомий вирок, порожній текст, дубль ключа."""
    renamed: tuple[Rename, ...] = ()
    """Інструменти, що змінили ім'я, лишивши інструкції (`rebind`).

    Ворота від цього ЧЕРВОНІ, хоч продукт і працює далі: перейменування --
    подія, яку людина мусить побачити, а не стан, у якому можна жити. Різниця
    між «працює» і «зелене» тут навмисна."""
...
class Rename:
    ...
    now: str
    shared: int
    """Скільки абзаців збіглося."""
    of: int
    """Скільки їх було у зниклого. `shared` з `of` -- і є доказ."""
...
class Digest:
    ...
    rows: tuple[str, ...] = ()
    dropped: int = 0
    """Скільки абзаців знято як не наші -- число їде в промпт рядком."""
    unjudged: int = 0
    """Скільки знято через відсутній вирок. Не нуль -- ворота вже червоні."""
    renamed: tuple[Rename, ...] = ()
    """Інструменти, чиї вироки взято під СТАРИМ іменем (`rebind`)."""

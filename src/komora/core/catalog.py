...
def sections_of(tree: Sequence[Node]) -> dict[str, str]:
    by_id = {node.id: node for node in tree}
    out: dict[str, str] = {}
    for node in tree:
        parent = by_id.get(node.parent_id or "")
        if parent is not None and parent.parent_id:
            out[node.slug or node.id] = parent.title
    return out
...
__all__ = ["display_roots", "kind_only", "kinds_of", "roots_of", "same_kind", "sections_of"]

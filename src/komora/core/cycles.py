...
class Trust(StrEnum):
    ...
    CYCLE = "cycle"
    """Цикл доведений: смуга, «ще ~N дн», «закінчується за циклом»."""

    SILENT = "silent"
    """Цикл доведений, але вид мовчить довше за SILENCE_LIMIT своїх циклів.
    Він справді закінчився — але сказати про це треба мовчанням, а не
    циклом: «давно не брав» замість «закінчується за циклом»."""

    RARE = "rare"
    """Циклу немає: проміжки стрибають або покупок замало. Числа немає
    взагалі — ні смуги, ні дати, ні «закінчилось». Рядок каже, ЯК саме
    береться."""

    ELSEWHERE = "elsewhere"
    """Проміжок між покупками рівний, але БІЛЬШИЙ за те, скільки вид взагалі
    може пролежати. Отже він описує не споживання, а нашу видимість: гість
    бере це ще десь. Окремо від RARE, бо причина протилежна -- там числа
    немає через розкид, тут воно є і саме тому не годиться."""

    NOT_RHYTHM = "not_rhythm"
    """Проміжок між покупками рівний і в межах строку зберігання -- а ритм
    покупок цей вид усе одно не описує: сезонне, з'їдається за раз, береться
    про запас чи про гостей. Вирок про сам ТОВАР, і дає його агент
    (`agent/sanity`), бо переліку такого в коді бути не може -- він протух би
    за місяць (#133).

    Окремо від `ELSEWHERE`, хоч наслідок однаковий (числа немає), бо ПРИЧИНА
    інша, а текст причини міняється разом із підставою (#208): там гість
    бере це ще десь, тут покупка просто не дорівнює споживанню."""

    SAID = "said"
    """Цикл назвав ГІСТЬ. Сильніший за будь-який висновок з чеків, бо він
    про інше: чек знає покупки в «Сільпо», гість знає свою кухню (#144)."""
...
class Rhythm:
    ...
    purchases: int
    """Скільки РІЗНИХ днів гість це купував. Не чеків: один похід дає кілька
    чеків (алкоголь і підписка йдуть окремими), і без цього проміжки
    засмічуються нулями."""

    trust: Trust
    cycle_days: int | None
    """Доведений цикл у днях, або None. None означає рівно «не знаємо»:
    показувати замість нього медіану, порахувану з двох стрибків, — це
    вигадана точність, яку гість не має як заперечити."""

    days_since: int | None
    receipts_days: int | None = None
    """Що бачать САМІ чеки: медіана проміжків, доведена вона чи ні. Окремо
    від `cycle_days` навмисно -- те число означає «доведено», а рядок
    «береш це не тільки тут» мусить назвати саме недоведене."""

    shortest: int | None = None
    longest: int | None = None
    """Найкоротший і найдовший проміжок. Вони і є доказом для рядка «береш
    нерівно»: «з твоїх чеків» без розподілу лишається твердженням (#79)."""

    beyond_keeps: bool = False
    """Чи довший НАШ проміжок за те, скільки одна покупка може пролежати.

    Рахується ЗАВЖДИ, а не лише в тій гілці, яка його читає, і саме в цьому
    сенс поля. Вирок агента (`NOT_RHYTHM`) стоїть у `rhythm` попереду стелі,
    тож рядок, у якого правдиві ОБИДВІ причини, до гілки `ELSEWHERE` не
    доходить за побудовою -- а питати гостя про такий рядок так само нема
    про що: чеки вже відповіли, що бачать не весь дім (`core/traits`,
    стан `NO`). Друга копія цього порівняння в місці, де ріжеться питання,
    розійшлася б з першою мовчки (#158)."""

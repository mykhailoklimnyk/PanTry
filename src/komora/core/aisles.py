...
class Aisled:
    ...
    of: Mapping[str, str]
    """Id рядка -> назва відділу. Рядок тут є ЗАВЖДИ: без кореня він іде в
    `CATCH_ALL`, бо відділ -- це фільтр, а рядок, якого не видно в жодному
    фільтрі, зникає з продукту мовчки (#38)."""
    order: tuple[Aisle, ...]
    """Відділи в порядку рейки. Порожньо -- дерева немає (без бази), і рейку
    показувати нема з чого."""

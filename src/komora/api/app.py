...
from komora.agent.refill import (
    RefillError,
    pick_answer,
    refill,
    take_cheaper,
...
from komora.core.location import HOME_DELIVERY, Address, Location
...
from komora.core.slots import moment_of as slot_moment
...
def _topup_refused(result: CheckoutResult, exc: HTTPException) -> CheckoutResult:
    why = str(exc.detail)
    log.warning("checkout.topup_refused", status=exc.status_code, why=why[:160])
    step = TraceStep(
        id="step-topup-refused",
        seq=max((step.seq for step in result.trace), default=0) + 1,
        tool="quota.counted",
        args={"status": exc.status_code},
        result_summary=f"добір під стіл не виконано: {why}",
        tag="кошик записано",
        tag_tone="warn",
    )
    return result.model_copy(update={"trace": [*result.trace, step]})
        ...
        async with SilpoMCP(token=guest.access, writes=True) as mcp:
            ...
            while (
                ...
                try:
                    ...
                    result = _topup_refused(result, exc)
                    break
...
async def _seen_receipts(mcp: SilpoMCP, guest: Guest, *, fresh: bool, place: Location) -> Receipts:
    if not fresh:
        seen = _remembered_receipts(guest)
        if seen is not None and _same_shelf(seen, place=place):
            return seen
...
def _same_shelf(seen: Receipts, *, place: Location | None) -> bool:
    if place is not None:
        want = place.branch_for(HOME_DELIVERY) or settings.branch_id
        if seen.branch_id != want:
            log.info("history.branch_changed", had=seen.branch_id or "-", want=want or "-")
            return False
    ends = slot_moment(seen.slot.get("end"), now=datetime.now(UTC))
    if ends is not None and ends <= datetime.now(UTC):
        log.info("history.slot_passed", ends=str(seen.slot.get("end")))
        return False
    return True
...
def _remembered_receipts(guest: Guest) -> Receipts | None:
    ...
    return seen.detached() if isinstance(seen, Receipts) else None
...
async def loop_pantry(guest: Guest, body: RefineRequest | None = None) -> Pantry:
    ...
    if body is not None and body.answers:
        ...
        taken = list(body.answers.items())[:PROBE_CEILING]
        if len(body.answers) > PROBE_CEILING:
            log.warning(
                "pantry.answers_capped",
                got=len(body.answers),
                kept=PROBE_CEILING,
                cut=[label for label, _ in list(body.answers.items())[PROBE_CEILING:]][:6],
            )
        for label, days in taken:
            under = body.kinds.get(label)
            if not under:
                continue
            await pantry_marks.save_cycle(get_pool(), guest.account, under, days)
            covers = list(body.covers.get(label, []))
            if len(covers) > PROBE_COVERS:
                log.warning(
                    "pantry.covers_capped", label=label[:40], got=len(covers), kept=PROBE_COVERS
                )
            answered[label] = (days, covers[:PROBE_COVERS])
...
async def adjust_pantry(adjustment: PantryAdjustment, guest: Guest, cold: bool = False) -> Pantry:
    if not guest.account:
        raise HTTPException(
            status_code=409,
    ...
    try:
        async with SilpoMCP(token=guest.access) as mcp:
            ...
            return await pantry_live(
                ...
                receipts=paper,
                ask=False,
                use_cache=not _cold_run(guest, cold),
                memory=True,
            )
    except AssemblyError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc

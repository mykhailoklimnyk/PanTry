...
from komora.core.spine import BATCH_MAX, MAX_TURNS, Halt, Progress, fresh, halt, trim
...
def _learned(before: Mapping[str, tuple[int, Any]], facts: Facts) -> tuple[str, ...]:
    return tuple(
        sorted(
            name
            for name in facts.names()
            if (origin := facts.origin(name)) is not None
            and fresh(before.get(name), origin.size, origin.value)
        )
    )

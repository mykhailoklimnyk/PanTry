...
from komora.mcp.client import MCPCallError, SilpoMCP, is_throttled
...
async def align_slot(mcp: SilpoMCP, snapshot: CartSnapshot, slot: dict[str, Any]) -> bool:
    ...
    if not start or not end:
        return False
    if snapshot.timeslot_start == start and snapshot.timeslot_end == end:
        return False
    await mcp.call(
...
async def hand_off(
    ...
    *,
    ...
    runner = Executor(
        ...
        writes_allowed=True,
        carried=done_before,
        fatal=(MCPCallError, AssemblyError),
    )
    ...
    return CheckoutResult(
        ...
        stock_cut=stock_cut,
        checkout_web_link=None if blockers else (after.checkout_web_link or CHECKOUT_URL),

...
class Made:
    ...
    tag_tone: Tone = "muted"
    tool: str = ""
    """Порт, яким крок СПРАВДІ скористався. Порожньо -- той, що назвав виклик.

    Потрібно це там, де порт відомий лише ПІСЛЯ виклику: моделей у нас пара
    з різними платниками (#294), і трейс мусить казати, ЯКА саме вирішувала
    -- інакше «модель обрала» описує обидві однаково. Ім'я кроку від цього
    не залежить: словник знає своє ім'я, а гість читає назву моделі."""
    prompt: str | None = None
    absent: str = ""
    """Чому крок НЕ дав факту -- його ВЛАСНИМИ словами.

    Порожньо -- крок спрацював. Інакше виконавець відмовляє від кроку цим
    текстом, а не своїм: «кошика в акаунті ще немає» і «збій кроку» -- різні
    відповіді гостю, і загальні слова виконавця стерли б різницю (#101: наше
    «недоступно» на екрані читається як факт про чуже API). Тон при цьому теж
    кроків: порожнеча нового акаунта жовтою бути не має права.
    """
    absent_kind: Refusal = Refusal.EMPTY
    """Якого РОДУ ця відмова -- законна порожнеча чи зламане (#311).

    Замовчування -- законна: крок, який сам ловить свій виняток і переказує
    його словами, мусить сказати це ЯВНО. Виводити рід з кольору ярлика було
    б тим самим, від чого стереже правило «колір не єдиний носій»: `warn`
    ставлять і там, де просто хочуть привернути око.
    """
...
class Executor:
    ...
    async def run_all(
        ...
        *,
        ...
        gained = {
            key: self.facts.put(key, value, step=author[key], batch=self.batch).told()

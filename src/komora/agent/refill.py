...
from komora.api.schemas import Declined, RefillRequest, TraceStep, Trimmed
from komora.core import models, quota
from komora.core import skills as core_skills
from komora.core.queries import covers
...
def missing_from(plan: Assembled, intents: list[str]) -> list[str]:
    ...
    return [
        ...
        if not any(
            covers(known, norm_name(intent)) or covers(norm_name(intent), known) for known in have
        )
    ]
    ...
    *,
        ...
        for line in ours:
            ...
            cut_fill.append(
                Trimmed(
                    ...
                    price=line.total,
                    reason=f"докинув це я під ціль {high:.0f} грн — на справжній ціні не влізло",
                )
            )
            money = settle(lines, terms, blockers=blockers)
    ...
    basket = plan.basket.model_copy(
        update={
            ...
            "fill_cut": [*plan.basket.fill_cut, *cut_fill],
...
async def top_up_after_twins(
    mcp: SilpoMCP,
    llm: Any,
    run: Assembled | CartRun,
    **kwargs: Any,
) -> Assembled | CartRun:
    if isinstance(run, CartRun) or llm is None:
        return run
    plan = plan_of(run)
    if not plan.basket.twins_dropped or plan.basket.budget is None:
        return run
    edges = band(Decimal(str(plan.basket.budget)))
    if Decimal(str(plan.basket.total)) >= edges.low:
        return run
    try:
        return await refill(
            mcp,
            llm,
            run,
            RefillRequest(intents=[]),
            trim_fill=True,
            label="замість близнюка",
            **kwargs,
        )
    except RefillError as exc:
        log.info("refill.after_twins_refused", error=str(exc))
        return run

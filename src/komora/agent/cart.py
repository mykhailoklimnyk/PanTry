...
from komora.core.mandate import Fork, build_comment, price_fork_comment
...
class CartSnapshot:
    ...
    """Кінець слота потрібен не для краси: пошук вимагає ОБИДВА кінці вікна,
    і запит із `end == start` віддає порожньо — тобто мовчки знімає заміни."""
    rows: tuple[dict[str, Any], ...]
    validations: tuple[dict[str, Any], ...]
    checkout_web_link: str | None
    """Куди «Сільпо» веде гостя оформлювати. З'являється НЕ завжди.

    Знято 15.08 двома викликами поспіль: на кошику з `timeslot.not_found`
    поля немає взагалі, а щойно кошик став валідним — воно приходить
    (`https://silpo.ua/checkout-new`). Тобто його відсутність означає
    «оформлювати ще нема чого», а не «API такого не вміє»."""
    checkout_mobile_link: str | None
    """Те саме для застосунку: коротке `link.silpo.ua/...`."""
    address: dict[str, Any]
    """Адреса кошика РІВНО як прийшла. Конструювати її руками не можна —
    `silpo_update_shopping_cart` вимагає віддати назад те, що дав сам."""
    shipments: tuple[dict[str, Any], ...]
    """Відвантаження без `products`: для оновлення слота потрібні лише
    companyId і branchId, а рядки там важать більше за все інше разом."""
    weight_kg: Decimal
    """`calculation.delivery.totalWeight` — вага з API, не з нашого розбору назв."""
    products_total: Decimal
    """`calculation.productsTotal` — скільки коштують САМІ рядки.

    Не `totalAfterDiscounts`: у ту суму входять доставка, сертифікати й
    бонуси, і поруч із нашою сумою рядків вона читалася б як розбіжність
    у розрахунку. Два різні числа в одному трейсі — гірше за одне."""
    discount: Decimal
    """`calculation.subDiscount` — скільки «Сільпо» зняло знижками.

    Наша економія рахується з `oldPrice` пошуку, тобто бачить лише акцію на
    полиці. Персональні промо, підписка й купони живуть на рівні КОШИКА, і
    поки позиції в ньому не лежать, їх не знає ніхто."""
    total_after_discounts: Decimal | None
    """`calculation.totalAfterDiscounts` — сума, яку гість справді заплатить
    (разом з доставкою). В описі інструмента сказано прямо: показувати
    ЦЕ число, а не `total`.

    `None` — кошика ще не читали або відповідь без розрахунку. Нуль тут був
    би гіршим за незнання: він читається як «безкоштовно»."""
    bonus_available: Decimal
    delivery_cost: Decimal | None
    """`calculation.delivery.total` — скільки доставка коштує НАСПРАВДІ.

    Наші пороги зі слота не знають ні про підписку, ні про персональні промо:
    у живому прогоні 14.08 ми надрукували 89 грн, поки кошик у тій самій
    відповіді казав 1 грн («4 доставки за 1 ₴» у Premium). Тому для
    прочитаного кошика авторитетне це число, а не наш розрахунок."""
    feedback_changes: Changes | None
    """`feedbackChanges` — галочка замін на рівні ЗАМОВЛЕННЯ (#49).

    Стоїть НАД нашим мандатом: при `disapprovedChanges` у збирача глобальна
    заборона міняти, тож погоджений ланцюжок біля полиці не спрацює. Тільки
    читаємо (`core/feedback.py`)."""
    feedback_contacts: Contacts | None
    """`feedbackContacts` — телефонувати чи ні. Теж тільки читаємо."""
...
async def read_cart(mcp: SilpoMCP) -> CartSnapshot:
    try:
        ...
        if not missing_resource(exc):
            raise
        raise AssemblyError(EMPTY_CART_NOTE) from exc
...
class CartRun:
    ...
    drops: tuple[str, ...] = ()
    """UUID рядків, від яких на полиці не лишилось нічого.

    Ре-валідація ставить заміну замість оригіналу, але сам оригінал у план
    не потрапляє — тобто в кошику він лишається назавжди, бо
    `silpo_add_or_update_cart_products` неназваних рядків не чіпає, а
    `quantity: 0` схема забороняє (`exclusiveMinimum: 0`). Гість отримував
    дубль і `blockers` замість посилання на оформлення (#113).
    """
...
async def commit(mcp: SilpoMCP, run: CartRun) -> dict[str, Any]:
    ...
    if fixes or second_drops:
        repaired = await apply_rewrites(mcp, after, fixes, drops=second_drops)
        log.info("cart.revalidated_after_write", fixes=len(fixes), drops=len(second_drops))
    return {
        "written": written,
    ...
    products = [
        {
            ...
            "addQuantity": False,
            **({"comment": line.mandate} if line.mandate else {}),
        }
        for line in lines

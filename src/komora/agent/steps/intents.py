...
class Compose:
    ...
    async def compose(self, bound: Mapping[str, Any]) -> Made:
        ...
        receipts, sizes = self.receipts, self.sizes
        ...
        for item in sorted(needs, key=lambda i: (-i.recent_receipts, -i.receipts)):
            ...
            if told is not None:
                if told.label in seen_labels:
                    folded += 1
                    folded_names.add(item.name)
                    continue
                seen_labels.add(told.label)
            kept.append(item)
        needs = kept
        needs.sort(
            key=lambda item: (
        ...
        if not intents:
            raise AssemblyError(
                ...
                if receipts == 0
                else "список порожній, і за циклами зараз нічого не закінчується"
                " — нема чого збирати"
            )
        ...
        if occasion.named:
            ...
            if not intents:
                raise AssemblyError(
                    f"під привід «{occasion.label}» з потреб тижня не лишилось нічого — "
        ...
        if self.budget is not None and occasion.fills_target:
            ...
            own_kind_keys: catalog.KindKeys | None = None
            if pool is not None:
                tracked = tracked_kinds(history)
                try:
                    own_nodes = await catalog_store.load(
                        pool, sorted({item.lager_id for item in tracked})
                    )
                except Exception as exc:
                    log.warning("target.kinds_unavailable", error=str(exc)[:200])
                else:
                    own_kind_keys = catalog.kind_keys(
                        ((item.name, item.lager_id) for item in tracked), own_nodes
                    )
            reserve = fill_pool(
                history,
                moment=moment,

...
class Home:
    ...
    async def name(self, bound: Mapping[str, Any]) -> Made:
        ...
        everyone = naming_candidates(kinds, self.said.listed.values())
        candidates = [name for name in everyone if name not in self.asked]
        again = len(everyone) - len(candidates)
        probe = Tally()
        await intent_names(
            None,
            candidates,
        ...
        self.asked.update(candidates)
        return Made(
            facts={"named": self.names},
            args={
                "кандидатів": len(everyone),
                "з кешу": tally.named_cached,
                **({"цим прогоном уже названо": again} if again else {}),
                "пачок": self.ground.packs,
            },
            summary=(
                f"названо {len(self.names)} з {len(everyone)}: "

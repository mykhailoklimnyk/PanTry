    ...
    *,
    ...
    try:
        ...
        trace.add(
            TABLE_STEP,
            "agent.occasion",
            {"докинути": names, "відмова": str(exc)[:200]},
            f"до столу не доклалось нічого: {exc}",
            decision="добір відмовився з причиною, і причина належить гостю: "
            "кошик лишається, як зібрано",
            tag="стіл: нема",
            tag_tone="warn",
        )
        return with_plan(
            ...
            replace(
                plan_of(run),
                basket=plan_of(run).basket.model_copy(update={"trace": trace.steps}),
            ),
        )
    log.info(
        "table.topped_up",

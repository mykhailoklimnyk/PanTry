...
"""Крок словника -> (метод `Home`, id кроку трейсу, порт). Порядок виконання
дає ПАЧКА моделі, а не цей перелік; `pantry.draw` тут немає навмисно -- він
інваріант `_turn`, а не крок плану."""
...
class Refined:
    ...
    note: str
    goal: Goal | None = None
    """`None` -- петля не крутилась: той самий вхід або немає моделі."""
    tokens: int = 0
    probes: tuple[Probe, ...] = ()

...
class AssemblyError(RuntimeError):
    """Збірку неможливо продовжити (немає слота тощо) — з людською причиною."""
...
class Naming:
    ...
    """Чи питали про напій ВЗАГАЛІ. Без цього поля «не напій» і «не питали»
    в кеші виглядають однаково — і бар на теплому кеші комори лишався б
    порожнім назавжди, при живому пиві в чеках. Кеш пережив би тут будь-яку
    міграцію: назви, названі до появи групи, лежать у ньому вічно."""
    ...
    *,
    ...
    if fresh and pool is not None and use_cache:
        try:
            ...
            _cache_names({name: _from_row(row) for name, row in known.items()})
...
async def intent_keeps(
    ...
    *,
    ...
    fresh = [label for label in labels if label and (not use_cache or label not in _KEEPS_CACHE)]
    ...
    *,
    ...
    for item in history:
        ...
        if item.said_cycle is not None:
            target.said_cycle = (
                item.said_cycle
                if target.said_cycle is None
                else min(target.said_cycle, item.said_cycle)
            )
...
class Tally:
    ...
    stored_papers: int = 0
    """Скільки паперових чеків прийшло зі СХОВИЩА, а не з мережі (#259)."""

    named_cached: int = 0
    """Скільки назв уже лежало в кеші -- у процесі або в базі."""

    named_asked: int = 0
    """Скільки довелось спитати в моделі. Нуль на теплому кеші -- норма."""
...
async def pantry_live(
    ...
    *,
    ...
    sense = await intent_sense(
        voice, naming_labels(names), pool=pool, use_cache=use_cache, batches=packs
    )
    ...
    hidden_ids = {item.lager_id for item in kinds if kind_key(item.name) in away}
    bar_ids = {
        item.lager_id
        for item in kinds
...
class PlanLine:
    ...
    qty: Decimal
    """Кількість Decimal, а не int: ваговий товар їде частками кроку
    (`addToBasketStep` 0.5 — це 500 г), і округлення до штук перетворило б
    півкіла філе на п'ять кілограмів або на нуль."""
    reason: str
...
async def _offline_pages(
    ...
    *,
    ...
    for _ in range(MAX_HISTORY_PAGES):
        ...
        if total is not None and taken >= int(total):
            break
...
class Agreed:
    ...
    chain: tuple[str, ...] = ()
    from_row: bool = False
    """Знайшлось ключем рядка ІСТОРІЇ, а не дослівним ключем наміру.

    Це і є мандат, погоджений у КОМОРІ: екран замін пише під дослівним
    ключем рядка кошика, а комора -- під видом з чека. Окремим числом у
    трейсі, бо «мандат з рядка доїхав» і «мандат з екрана замін доїхав» --
    різні твердження, і одне число на обидва не довело б жодного."""
...
async def assemble_list(
    ...
    *,
    ...
    if llm is not None:
        ...
        def _state() -> dict[str, Any]:
            ...
            if request.budget is not None:
                ...
                said["до верхньої межі, грн"] = f"{edges.high - total:.0f}"
            return said
    ...
    def cut(intent: str, why: str) -> None:
        nonlocal fill_cut
        line = by_intent[intent]
        lines.remove(line)
        if intent in fill_intents:
            fill_cut += 1
            fill_cut_names.append(intent)
            return
        trimmed.append(
            Trimmed(intent=intent, name=line.product["name"], price=line.total, reason=why)
        )
    ...
    if request.budget is not None and budget_now is not None:
        ...
        cuts_budget = 0
        while money.total > limit and queue:
            cut(*queue.pop(0))
            money = settle(lines, terms)
            cuts_budget += 1
        ...
        for line in spare:
            if money.total <= limit:
                break
            cut(line.intent, "понад межу після черги агента — знімаю найдорожче з докинутого")
            money = settle(lines, terms)
            cuts_budget += 1
    ...
    while over_limit(weight.kg, terms.max_weight_kg) > 0 and queue:
        ...
        cut(intent, f"{why}. Разом кошик важчий за ліміт слота")
        heavy_cuts += 1
        money = settle(lines, terms)
        weight = weigh(lines)
    if heavy_cuts:

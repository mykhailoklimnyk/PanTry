...
class WriteBlocked(RuntimeError):
    """Клієнт без права запису спробував змінити стан. Навмисно не підклас
    MCPCallError: це не збій виклику, а спрацьований запобіжник."""
...
class SilpoMCP:
    ...
    async def _dial(
        ...
        if last_error is not None and _is_token_rejected(last_error):
            raise TokenRejected(tool, str(last_error), attempts=attempts_allowed)
        raise MCPCallError(tool, str(last_error), attempts=attempts_allowed)

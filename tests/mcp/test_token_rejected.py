...
def _muted(status: int | None):
    from komora.mcp.client import SilpoMCP

    mcp = SilpoMCP(token="х")
    mcp._session = _Mute()  # type: ignore[assignment]
    mcp._last_status = status
    return mcp


async def test_a_token_revoked_mid_run_is_a_dead_token_not_a_broken_shop():
    mcp = _muted(401)

    with pytest.raises(TokenRejected) as caught:
        await mcp.call("silpo_get_my_offline_orders", {})

    assert not isinstance(caught.value, MCPCallError)
    assert "HTTP 401" in str(caught.value)


async def test_a_mute_server_crash_stays_a_crash():
    mcp = _muted(500)

    with pytest.raises(MCPCallError) as caught:
        await mcp.call("silpo_get_my_offline_orders", {})

    assert not isinstance(caught.value, TokenRejected)


async def test_the_error_counts_the_attempts_that_happened():
    mcp = _muted(500)
    ...
    assert "(спроб: 1)" in str(caught.value)

...
async def test_not_a_drink_is_written_down_as_a_word_not_as_a_hole():
    llm = _NamingLLM([{"name": "Лаваш", "intent": "лаваш", "subtype": None, "drink": None}])
    pool = _Pool()
...
async def test_a_word_the_bar_does_not_know_means_not_a_drink():
    llm = _NamingLLM([{"name": "Лаваш", "intent": "лаваш", "subtype": None, "drink": "пиво"}])
...
class TestBarNeMiniaieMitkyVydu:

    def _stored(self, intent: str, subtype: str | None = None) -> _Pool:
        return _Pool(
            [
                {
                    "name_sha": store.fingerprint("Пиво Hike Blanche"),
                    "intent": intent,
                    "subtype": subtype,
                    "drink": None,
                }
    ...
    async def test_the_new_intent_does_not_reach_the_eternal_cache(self) -> None:
        forget_intents()
        pool = self._stored("пиво")
        llm = _NamingLLM(
            [
                {
                    "name": "Пиво Hike Blanche",
                    "intent": "напій",
                    "subtype": "світлий",
                    "drink": "light",
                }
        ...
        named = await intent_names(llm, ["Пиво Hike Blanche"], pool=pool, need_drink=True)

        assert named["пиво hike"].intent == "пиво", "мітка лишається тією, що вже в базі"
        assert named["пиво hike"].drink is not None, "а групу напою бар таки отримує"
        written = [rows for kind, rows in pool.written if kind == "many"]
        assert written, "запис мусить статись: без нього бар питатиме щоразу"
        assert written[-1][0]["intent"] == "пиво"

    async def test_the_subtype_is_kept_too(self) -> None:
        forget_intents()
        pool = self._stored("пиво", "світле")
        llm = _NamingLLM(
            [{"name": "Пиво Hike Blanche", "intent": "пиво", "subtype": "біле", "drink": "light"}]
        )

        named = await intent_names(llm, ["Пиво Hike Blanche"], pool=pool, need_drink=True)

        assert named["пиво hike"].subtype == "світле"

    async def test_a_name_nobody_named_yet_is_written_whole(self) -> None:
        forget_intents()
        pool = _Pool([])
        llm = _NamingLLM(
            [{"name": "Пиво Hike Blanche", "intent": "пиво", "subtype": "світле", "drink": "light"}]
        )

        named = await intent_names(llm, ["Пиво Hike Blanche"], pool=pool, need_drink=True)

        assert named["пиво hike"].intent == "пиво"
        assert named["пиво hike"].subtype == "світле"

    async def test_the_kept_label_says_so(self) -> None:
        forget_intents()
        pool = self._stored("пиво")
        llm = _NamingLLM(
            [{"name": "Пиво Hike Blanche", "intent": "напій", "subtype": None, "drink": "light"}]
        )

        with capture_logs() as written:
            await intent_names(llm, ["Пиво Hike Blanche"], pool=pool, need_drink=True)

        assert [one for one in written if one.get("event") == "naming.label_kept"]

    async def test_the_same_label_says_nothing(self) -> None:
        forget_intents()
        pool = self._stored("пиво", "світле")
        llm = _NamingLLM(
            [{"name": "Пиво Hike Blanche", "intent": "пиво", "subtype": "світле", "drink": "light"}]
        ...
        assert not [one for one in written if one.get("event") == "naming.label_kept"]

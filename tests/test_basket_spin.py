...
async def test_the_replan_sees_the_basket_and_the_money(mcp) -> None:  # noqa: F811
    ...
    assert "зібрано, грн" in state and "до верхньої межі, грн" not in state
    assert state["кроки оберту"] == "decide.chain, decide.loop, decide.pick"


async def test_the_replan_is_told_the_limit_the_code_actually_cuts_to(mcp) -> None:  # noqa: F811
    llm = _Planning([{"steps": [], "stop": True}])
    ...
    state = llm.asked[0]["стан"]
    built = Decimal(state["зібрано, грн"])
    assert state["до межі різу, грн"] == f"{Decimal(1600) - built:.0f}"
    assert "до верхньої межі, грн" not in state, "вісь, якої код не ріже, не називається"

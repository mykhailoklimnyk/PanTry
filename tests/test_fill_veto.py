...
@pytest.mark.anyio
async def test_a_touch_on_a_bar_pick_lands_the_beer_in_the_basket(tmp_path) -> None:
    from komora.agent.basket import BAR_ASK_INTENT
    from komora.agent.refill import pick_answer
    ...
    assert any(q.intent == BAR_ASK_INTENT for q in event.basket.questions)
    filled = await pick_answer(event, intent=BAR_ASK_INTENT, article="701", now=NOW)
    assert any(str(line.product["externalProductId"]) == "701" for line in filled.lines)
    assert all(q.intent != BAR_ASK_INTENT for q in filled.basket.questions)

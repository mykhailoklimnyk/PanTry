...
async def test_the_rewritten_row_carries_a_mandate_that_fits_the_field():
    ...
    await apply_rewrites(mcp, snapshot, [_line(202, "Заміна", mandate=long)])

    written = mcp.args_of("silpo_add_or_update_cart_products")
    assert len(written["products"][0]["comment"]) <= COMMENT_MAX
...
async def _short_run(mcp: FakeCart, line: PlanLine) -> CartRun:
    snapshot = await read_cart_by_id(mcp, CART_ID)
    mcp.calls.clear()
    return CartRun(
        assembled=_assembled([line]),
        snapshot=snapshot,
        chains={"201": (Alternative("202", "Заміна", Source.HISTORY, price=Decimal(60)),)},
        cards={"202": {"id": _uuid(202), "companyId": _uuid(1), "branchId": _uuid(2)}},
    )


async def test_the_second_round_cuts_the_original_to_what_is_left_on_the_shelf():
    from komora.agent.cart import commit

    mcp = FakeCart(rows=[_row(201, "Оригінал", qty=5, stock=2)])
    run = await _short_run(mcp, replace(_line(201, "Оригінал"), qty=Decimal(5)))

    await commit(mcp, run)

    writes = [args for tool, args in mcp.calls if tool == "silpo_add_or_update_cart_products"]
    assert len(writes) == 2, "другого кола не було взагалі"
    second = {str(row["productId"]): row["quantity"] for row in writes[1]["products"]}
    assert second.get(_uuid(201)) == Decimal(2), "оригінал лишився зі старою кількістю"
    assert second.get(_uuid(202)) == Decimal(3), "заміна не закрила решту"


async def test_the_second_round_trim_is_a_copy_not_an_edit_of_the_plan():
    from komora.agent.cart import commit

    mcp = FakeCart(rows=[_row(201, "Оригінал", qty=5, stock=2)])
    line = replace(_line(201, "Оригінал"), qty=Decimal(5))
    run = await _short_run(mcp, line)

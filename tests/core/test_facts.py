...
def test_a_rewritten_fact_moves_its_origin_to_the_last_step():
    ...
    facts.put("candidates", {"а": [1]}, step="shelf.search", batch=1)
    facts.put("candidates", {"а": [1], "б": [2]}, step="shelf.by_article", batch=2)

    origin = facts.origin("candidates")
    assert origin.step == "shelf.by_article" and origin.batch == 2 and origin.size == 2
...
def test_the_ground_truth_is_sizes_and_origins_not_bodies():
    ...
    facts.put("candidates", {"молоко": [{"name": "..."}] * 20}, step="shelf.search", batch=1)

...
def test_the_strict_axis_keeps_an_unknown_article_to_itself():
    keys = kind_keys([*WATER_ROWS, ("Вода питна Моршинка", "w9")], WATER, unknown_merges=False)
    assert keys.key("Вода питна Моршинка") == "вода·Вода питна Моршинка"
    assert len(keys.by_word["вода"]) == 3


def test_the_strict_axis_still_folds_what_the_node_proves_the_same():
    rows = [("Томат", "t1"), ("Томат Green Agro", "t2"), ("Томат рожевий Укр", "t3")]
    node = frozenset({"tomaty-4100"})
    keys = kind_keys(rows, dict.fromkeys(("t1", "t2", "t3"), node), unknown_merges=False)
    assert len({keys.key(name) for name, _ in rows}) == 1
...
def test_a_section_is_the_parent_of_a_node_unless_that_parent_is_a_root():
    from komora.core.catalog import sections_of
    from komora.core.dictionary import Node

    tree = [
        Node(id="r", title="Хліб та випічка", slug="khlib-ta-vypichka"),
        Node(id="m", title="Хлібобулочні вироби", parent_id="r", slug="khlibobulochni"),
        Node(id="a", title="Хліб", parent_id="m", slug="khlib"),
        Node(id="b", title="Булки", parent_id="m", slug="bulky"),
        Node(id="c", title="Торти", parent_id="r", slug="torty"),
        Node(id="d", title="Без слага", parent_id="m"),
    ]

    assert sections_of(tree) == {
        "khlib": "Хлібобулочні вироби",
        "bulky": "Хлібобулочні вироби",
        "d": "Хлібобулочні вироби",
    }

...
"""ЯДРО, ЯКЕ КОД ВИКОНУЄ ЗАВЖДИ (#365, замір 08.09).

`decide.chain` не з'явився в ЖОДНОМУ з 20 планів моделі на чотирьох формах
задачі, і валідатор через це різав `economy.settle` («немає входу: lines»)
десять разів, `cart.write` п'ять, `decide.loop` три. Продукт не ламався --
виконавець кошика стартує з плану КОДУ, у якому ці кроки вже стоять, а план
моделі лише додається, -- але третина плану їхала в нікуди за наші токени.
Після того, як промпт назвав їх фактом, різ «немає входу» зник цілком: три
проби поспіль дали нуль.
"""

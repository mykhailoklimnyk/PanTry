...
def test_a_word_inside_another_word_is_not_the_same_word():
    assert not covers("Чайний напій", "чай")
    assert not covers("Медальйони свинячі", "мед")
    assert not covers("сирники", "сир")


def test_a_shorter_name_inside_a_longer_one_still_counts():
    assert covers("молоко ферма 2,5%", "молоко")
    assert covers("сир кисломолочний", "сир")


def test_the_words_must_stand_together_and_in_order():
    assert covers("сир кисломолочний простонаше", "кисломолочний простонаше")
    assert not covers("сир кисломолочний простонаше", "сир простонаше")
    assert not covers("сир", "сир кисломолочний")


def test_an_empty_part_covers_nothing():
    assert not covers("сир", "")
    assert not covers("", "")

...
def test_what_is_missing_is_counted_to_the_LOW_edge():
    ...
    assert edges.over_by(money("3400")) == money("100.00")
...
def test_only_the_rows_that_actually_save_become_options():
    ways = overshoot(money("2220"), band=band(money("1600")), rows=_rows("520", "129"))
...
def test_the_dearest_row_stands_first():
    ways = overshoot(money("2220"), band=band(money("1600")), rows=_rows("520", "980", "700"))
...
def test_the_number_of_options_has_a_ceiling():
    ways = overshoot(
        money("2220"), band=band(money("1600")), rows=_rows("520", "530", "540", "550", "560")
    )
...
def test_without_a_single_saving_row_there_is_nothing_to_ask():
    ways = overshoot(money("2220"), band=band(money("1600")), rows=_rows("20", "30"))

    assert not ways.asks() and ways.drops == ()
    assert ways.target == money("2100")


def test_the_raised_target_is_the_smallest_round_one_that_holds_the_basket():
    ways = overshoot(money("2220.04"), band=band(money("1600")), rows=_rows("999"))

    assert ways.target == money("2100")
    assert band(ways.target).high >= money("2220.04"), "запропонована ціль справді вміщає кошик"


def test_a_basket_exactly_at_the_top_edge_is_not_over_it():
    ways = overshoot(money("1760"), band=band(money("1600")), rows=_rows("10"))

...
def test_a_fact_that_was_not_there_is_always_new() -> None:
    assert fresh(None, 0, {})


def test_a_relaid_subset_is_not_knowledge() -> None:
    was = (2, {"молоко": {"chosen_id": "1"}, "хліб": {"chosen_id": "2"}})

    assert not fresh(was, 1, {"молоко": {"chosen_id": "1"}})
    assert not fresh(was, 2, dict(was[1]))


def test_a_new_key_or_a_changed_one_is_knowledge() -> None:
    was = (2, {"молоко": {"chosen_id": "1"}, "хліб": {"chosen_id": "2"}})

    assert fresh(was, 1, {"кава": {"chosen_id": "9"}}), "новий намір -- це відповідь"
    assert fresh(was, 1, {"молоко": {"chosen_id": "7"}}), "інший вибір під тим же наміром"


def test_a_fact_that_grew_in_place_is_knowledge() -> None:
    grown = {"молоко": [1, 2]}

    assert fresh((1, grown), 2, grown)


def test_a_shape_we_cannot_compare_deeply_stays_new() -> None:
    assert fresh((3, [1, 2, 3]), 3, [4, 5, 6])
    assert not fresh((3, [1, 2, 3]), 3, [1, 2, 3])


def test_a_set_that_only_lost_members_is_not_knowledge() -> None:
    assert not fresh((3, {"а", "б", "в"}), 2, {"а", "б"})
    assert fresh((3, {"а", "б", "в"}), 2, {"а", "г"})

...
def test_an_axis_that_does_not_fit_the_field_is_dropped_whole():
    ...
    from komora.core.mandate import COMMENT_MAX, price_fork, price_fork_comment

    comment = price_fork_comment(price_fork(Decimal(100)), kind="дуже довга вісь " * 40)
    assert len(comment) <= COMMENT_MAX
...
def test_an_axis_that_fills_the_field_exactly_still_fits():
    ...
    assert len(price_fork_comment(price_fork(price), kind="х" * longest)) == COMMENT_MAX
...
def test_the_built_mandate_needs_no_second_cut_and_names_what_it_dropped():
    from komora.core.mandate import COMMENT_MAX, fit_comment

    chain = [alt(f"Сир витриманий довгої назви номер {index}") for index in range(1, 7)]
    mandate = build_comment(chain=chain, shelf_life="не менше ніж до 19.08")

    assert len(mandate.comment) <= COMMENT_MAX
    assert fit_comment(mandate.comment) == mandate.comment
    assert mandate.dropped
    for name in mandate.dropped:
        assert name not in mandate.comment

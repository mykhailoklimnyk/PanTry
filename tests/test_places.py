...
async def test_search_names_a_street_without_a_house_as_unconfirmed(stand):
    from komora.api.schemas import PlaceQuery
    ...
    stand.payloads = {
        **LIVE,
        "silpo_find_address": {
            "addresses": [
                {**FOUND_ROW, "street": "вулиця Хрещатик", "houseNumber": "30/1"},
                {
                    "address": "Київ, вулиця Хрещатик",
    ...
    found = await api.search_place(PlaceQuery(text="Київ, Хрещатик, 999"), GUEST)

    assert [(option.house, option.confirmed) for option in found] == [
        ("30/1", True),
        (None, False),
    ]


async def test_chosen_address_keeps_its_street_and_house(stand):
    from komora.api.schemas import PlaceChoice

    await api.choose_place(
        PlaceChoice(
            label="Київ, вулиця Хрещатик, 30/1",
            latitude=50.44,
            longitude=30.52,
            city="Київ",
            street="вулиця Хрещатик",
    ...
    assert (address.city, address.street, address.house) == ("Київ", "вулиця Хрещатик", "30/1")

...
def test_a_docstring_that_is_the_whole_body_stays() -> None:
...
def test_the_exception_is_the_module_docstring_not_the_file() -> None:
    source = MODULE_AND_FUNCTION
    kept, count = strip_python(source, docstrings=True, keep_module=True)
    assert kept.startswith('"""Опис модуля."""')
    assert "Чому саме так" not in kept
    assert count == 1

    muted, count = strip_python(source, docstrings=True)
    assert not muted.startswith('"""')
    assert count == 2
...
def test_files_named_as_untouchable_are_really_there() -> None:
    for path in AS_IS | PUBLIC_DOCS | KEEP_MODULE_DOCSTRING:
        assert (ROOT / path).exists(), f"{path} у списку є, у дереві немає"

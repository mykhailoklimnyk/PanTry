...
async def test_the_endpoint_hands_the_remembered_history_to_the_refill(stand, monkeypatch):
    ...
    seen = Receipts(slot={}, branch_id=None, history=[], count=7)
    history_memory.remember(seen, owner=GUEST.owner)
    await api.refill_basket(first.run_id, RefillRequest(), GUEST, None)
    passed = handed["receipts"]
    assert passed is not None, "забутий аргумент повернув би три перечитування"
    assert passed is not seen, "читач править СВОЮ копію"
    assert passed.count == seen.count
...
async def test_an_answer_does_not_push_the_basket_over_the_corridor(stand):
    ...
    assert [item.intent for item in filled.basket.fill_cut] == step.args["знято під межу"]
    assert filled.basket.trimmed == []
    assert all(item.name and item.price is not None for item in filled.basket.fill_cut)
...
async def test_a_refused_table_top_up_names_its_reason_in_the_same_trace(stand, monkeypatch):
    plan = await _plan(stand, mode="event", occasionPeople=4, budget=3000)

    async def _refuse(*_args, **_kwargs):
        raise RefillError("агент не взяв: «Товар 7» -- не той вид")

    monkeypatch.setattr("komora.agent.table.refill", _refuse)

    filled = await top_up_table(
        stand, _TableLLM(["Товар 7"]), plan, occasion_of("event", 4), now=NOW
    )

    assert [line.product["name"] for line in filled.lines] == [
        line.product["name"] for line in plan.lines
    ], "кошик не змінився -- відмова добору це не поломка збірки"
    refused = [s for s in filled.basket.trace if s.id == "step-table"][-1]
    assert "до столу не доклалось нічого" in refused.result_summary
    assert "не той вид" in refused.args["відмова"] and refused.tag == "стіл: нема"
...
def test_a_word_inside_another_word_does_not_count_as_already_in_the_cart():
    plan = SimpleNamespace(
        lines=[SimpleNamespace(intent="чайний напій", product={"name": "Чайний напій Ліптон"})]
    )

    assert missing_from(plan, ["чай"]) == ["чай"]
    assert missing_from(plan, ["мед"]) == ["мед"]


def test_a_shorter_name_of_the_same_product_is_still_dropped():
    plan = SimpleNamespace(
        lines=[SimpleNamespace(intent="молоко ферма", product={"name": "Молоко Ферма 2,5%"})]
    )

    assert missing_from(plan, ["молоко", "кефір"]) == ["кефір"]

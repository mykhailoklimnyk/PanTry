...
async def test_no_cart_at_all_refuses_with_the_same_words(tmp_path):
    ...
    _write_fixtures(tmp_path, _cart([]))
    said = tmp_path / "silpo_get_my_shopping_cart.json"
    said.write_text('{"exists": false}', encoding="utf-8")
...
async def test_a_cart_answer_that_names_nothing_is_silence_not_emptiness(tmp_path):
    from komora.agent.cart import UNNAMED_CART_NOTE
    ...
    assert str(refusal.value) == UNNAMED_CART_NOTE

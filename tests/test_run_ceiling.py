...
async def test_a_failed_run_writes_why_it_failed(pool: _Pool):
    _priced()
    guest = _Guest()

    with pytest.raises(HTTPException):
        async with quota.counted(guest, kind="basket", meter=_meter()):
            raise HTTPException(status_code=409, detail="за циклами нічого не закінчується")

    async with quota.counted(guest, kind="basket", meter=_meter()):
        pass

    failed, done = pool.rows.rows
    assert failed["ok"] is False
    assert failed["error"] == "409: за циклами нічого не закінчується"
    assert done["ok"] is True
    assert done["error"] is None


def test_the_failure_text_names_the_class_when_there_is_no_detail():
    assert quota.failure_text(KeyError("x")) == "KeyError: 'x'"
    assert len(quota.failure_text(RuntimeError("я" * 1000))) == quota.FAILURE_CHARS

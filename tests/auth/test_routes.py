...
def _alive(**fields: object) -> GuestSession:
    return GuestSession(expires_at=datetime.now(UTC) + timedelta(days=30), **fields)  # type: ignore[arg-type]
...
async def test_whoami_names_the_llm_key_cookie_on_every_state_poll(key):
    ...
    guest = _alive(access="токен", issued_at=NOW)
...
async def test_valid_session_passes_the_token_through(key):
    guest = _alive(access="токен-гостя")

...
from komora.agent.basket import HistoryItem, own_over_promo, pick_schema, pick_system
...
from komora.agent.twins import TWINS_SCHEMA, TWINS_SYSTEM
from komora.agent.twins import payload as twins_payload
from komora.agent.twins import verdicts_of as twins_verdicts
...
from komora.core.twins import judge as twins_judge
    ...
    "наміри": [
        {
            ...
            "з_історії": [
                {
                    "назва": "Томат Гордій Черрі",
                    "артикул": "411223",
                    "чеків_усього": 6,
                    "чеків_за_90_днів": 5,
                    "звична_кількість": 1,
                    "акційна_звичка": "береш по акції: 5 з 6",
                },
                {
                    "назва": "Томат",
                    "артикул": OWN_PROMO_PICK,
                    "чеків_усього": 6,
                    "чеків_за_90_днів": 3,
                    "звична_кількість": 1,
                    "акційна_звичка": None,
        ...
        copy.deepcopy(PROMO_ASKED["наміри"][0]),
    ],
}
OWN_PROMO_PROMPT, OWN_PROMO_SYSTEM, OWN_PROMO_MARKS = _skill_case(OWN_PROMO_ASKED)


def _own_promo_shown(data: dict) -> str:
    asked = OWN_PROMO_ASKED["наміри"][0]
    options = [
        {
            "externalProductId": str(card["id"]),
            "name": str(card["назва"]),
            "price": card["ціна"],
            "oldPrice": card["стара_ціна"],
        }
        for card in asked["кандидати"]
    ]
    owned = {
        str(row["артикул"]): HistoryItem(
            lager_id=str(row["артикул"]),
            name=str(row["назва"]),
            unit="кг",
            receipts=int(row["чеків_усього"]),
            qty_total=Decimal(int(row["чеків_усього"])),
            recent_receipts=int(row["чеків_за_90_днів"]),
            qty_recent=Decimal(int(row["чеків_за_90_днів"])),
        )
        for row in asked["з_історії"]
    }
    pick = _by_intent(data, OWN_PROMO_MARKS).get("Томат") or {}
    said = _digits(str(pick.get("chosen_id") or ""))
    chosen = next((p for p in options if p["externalProductId"] == said), None)
    if chosen is None:
        return f"модель: {said or 'порожньо'}"
    own = own_over_promo(
        "Томат",
        chosen,
        options,
        hint=owned.get(OWN_PROMO_PICK),
        owned=owned,
        why=str(pick.get("why") or ""),
        auto=True,
    )
    if own is None:
        return f"модель: {said}"
    return f"модель: {said} → код: {own['externalProductId']}"
...
def _days(count: int, start: int = 0) -> frozenset[str]:
    from datetime import date, timedelta

    first = date(2026, 1, 1)
    return frozenset((first + timedelta(days=start + day)).isoformat() for day in range(count))
...
TWIN_WATERS = Group(
    ...
    rows=(
        Row(
            intent="Вода дитяча Малятко спеціальна пробка",
            article="386913",
            name="Вода дитяча «Малятко» зі спеціальною пробкою",
            phrase="вода питна",
            own_article=True,
            receipts=3,
            recent_receipts=3,
            unit_price="19.49 грн/шт",
            came_from="звичне з історії",
            bought=_days(7, 0),
        ),
        Row(
            intent="Вода питна Моршинка Спорт негазована пет",
            article="369992",
            name="Вода мінеральна «Моршинська» спорт негазована",
            phrase="вода питна",
            own_article=True,
            receipts=3,
            recent_receipts=3,
            unit_price="30.99 грн/шт",
            came_from="звичне з історії",
            bought=_days(37, 5),
        ),
    ),
...
TWIN_TOMATO_PROMPT = twins_payload([TWIN_TOMATOES])
TWIN_WATER_PROMPT = twins_payload([TWIN_WATERS])


def _twin_verdict(data: dict, group: Group) -> Verdict | None:
    known, _stray = twins_verdicts(data, [group])
    return known.get(group.key)


def _twin_same_ok(data: dict) -> bool:
    verdict = _twin_verdict(data, TWIN_TOMATOES)
    return verdict is not None and verdict.same and _digits(verdict.keep) == "32589"


def _twin_apart_ok(data: dict) -> bool:
    verdict = _twin_verdict(data, TWIN_WATERS)
    return verdict is not None and not verdict.same


def _twin_shown(data: dict) -> str:
    rows = [item for item in (data.get("pairs") or []) if isinstance(item, dict)]
    return (
        ", ".join(
            f"{item.get('key') or '?'}="
            + ("одна→" + str(item.get("keep") or "?") if item.get("same") else "різні")
            + (f" зняти {item.get('drop')}" if item.get("drop") else "")
            for item in rows
        )
        or "нічого"
    )
...
def _twin_said(data: dict) -> str:
    rows = [item for item in (data.get("pairs") or []) if isinstance(item, dict)]
    return " ".join(str(item.get("why") or "") for item in rows)


TWIN_FOREIGN_TOMATOES = Group(
    key="томати",
    rows=(
        Row(
            intent="Томат",
            ...
            unit_price="71.77 грн/кг",
            promo="було 82.49, стало 71.77",
            came_from="акційна ціна: було 82.49, стало 71.77. вагове: звична вага з чеків — 0,4 кг",
        ),
        Row(
            intent="Томат Azura Черрі сливка",
            article="1022653",
            name="Томат Green Agro сливка червоний",
            phrase="томати",
            unit_price="68.52 грн/шт",
            promo="було 76.99, стало 68.52",
            came_from="акційна ціна: було 76.99, стало 68.52",
        ),
    ),
)

TWIN_LEMONADES = Group(
    key="лимонад",
    rows=(
        Row(
            intent="Напій Geo Natura Лимонад Кремовий скло",
            article="789516",
            name="Напій Geo Natura Лимонад крем-сода",
            phrase="лимонад",
            own_article=True,
            receipts=5,
            recent_receipts=5,
            unit_price="45.99 грн/шт",
            promo="було 62.99, стало 45.99",
            came_from="акційна ціна: було 62.99, стало 45.99",
        ),
        Row(
            intent="Напій Geo Natura Лимонад груша скло",
            article="739664",
            name="Напій Geo Natura «Лимонад лимон»",
            phrase="лимонад",
            own_article=True,
            receipts=3,
            recent_receipts=0,
            unit_price="49.99 грн/шт",
            promo="було 61.99, стало 49.99",
            came_from="брав це раніше, але не останнім часом",
        ),
    ),
)

TWIN_FOREIGN_PROMPT = twins_payload([TWIN_FOREIGN_TOMATOES])
TWIN_LEMONADE_PROMPT = twins_payload([TWIN_LEMONADES])


TWIN_BREADS = Group(
    key="хлібобулочні вироби",
    rows=(
        Row(
            ...
            receipts=8,
            recent_receipts=3,
            unit_price="62.46 грн/шт",
            came_from="закінчується за циклом ~3 дн",
            bought=_days(9, 0),
        ),
        Row(
            intent="Булка подова Французька зернова",
            article="596031",
            name="Булка подова Французька зернова",
            phrase="булка",
            own_article=True,
            receipts=4,
            recent_receipts=1,
            unit_price="47.99 грн/шт",
            promo="було 65.49, стало 47.99",
            came_from="закінчується за циклом ~5 дн",
            bought=_days(18, 100),
        ),
        Row(
            intent="Багет подовий Французький Люкс",
            article="375210",
            name="Багет подовий Французький Люкс",
            phrase="багет",
            own_article=True,
            receipts=3,
            recent_receipts=1,
            unit_price="45.49 грн/шт",
            came_from="закінчується за циклом ~3 дн",
            bought=_days(10, 200),
        ),
    ),
...
TWIN_VEG = Group(
    ...
    rows=(
        Row(
            ...
            receipts=6,
            recent_receipts=3,
            unit_price="51.49 грн/кг",
            bought=_days(35, 0),
        ),
        Row(
            intent="Огірок екстра",
            article="142342",
            name="Огірок Екстра",
            phrase="огірки",
            own_article=True,
            receipts=11,
            recent_receipts=4,
            unit_price="82.49 грн/кг",
            bought=_days(24, 19),
        ),
        Row(
            intent="Перець червоний",
            article="32887",
            name="Перець червоний",
            phrase="перець солодкий",
            own_article=True,
            receipts=15,
            recent_receipts=5,
            unit_price="92.99 грн/кг",
        ),
    ),
    by_section=True,
)

TWIN_BREAD_PROMPT = twins_payload([TWIN_BREADS])
TWIN_VEG_PROMPT = twins_payload([TWIN_VEG])


def _twin_dropped(data: dict, group: Group) -> int:
    verdict = _twin_verdict(data, group)
    if verdict is None:
        return -1
    return len(twins_judge([group], {group.key: verdict}).dropped)


def _twin_bread_ok(data: dict) -> bool:
    return _twin_dropped(data, TWIN_BREADS) >= 1


def _twin_veg_ok(data: dict) -> bool:
    return _twin_dropped(data, TWIN_VEG) == 0


def _twin_foreign_ok(data: dict) -> bool:
    verdict = _twin_verdict(data, TWIN_FOREIGN_TOMATOES)
    return verdict is not None and verdict.same and _digits(verdict.keep) in {"455724", "1022653"}


def _twin_lemonade_ok(data: dict) -> bool:
    verdict = _twin_verdict(data, TWIN_LEMONADES)
    return verdict is not None and verdict.same and _digits(verdict.keep) in {"789516", "739664"}
...
Обери товар під намір.""",
    ...
    Scenario(
        key="своє_проти_акції",
        prompt=OWN_PROMO_PROMPT,
        system=OWN_PROMO_SYSTEM,
        schema=pick_schema(with_queue=False),
        check=lambda v, ask: False,
        verdict=lambda data: _chose(data, OWN_PROMO_MARKS, "Томат") == OWN_PROMO_PICK,
        marks=OWN_PROMO_MARKS,
        shown=_own_promo_shown,
        note="пастка: знижка на ЧУЖОМУ товарі, а акційна звичка -- у сусіднього виду; "
        "свій артикул гостя дешевший і буквальний; колонка «обрано» каже, що з "
        "відповіді робить `own_over_promo` -- вердикт лишається про МОДЕЛЬ",
    ),
    Scenario(
        key="сира",
        prompt=RAW_PROMPT,
    ...
    Scenario(
        key="близнюки_томат",
        prompt=TWIN_TOMATO_PROMPT,
        system=TWINS_SYSTEM,
        schema=TWINS_SCHEMA,
        check=lambda v, ask: False,
        verdict=_twin_same_ok,
        shown=_twin_shown,
        said=_twin_said,
        note="пастка: два рядки одного виду -- одна потреба, і лишити треба свій "
        "артикул з чеків, а не заміну",
    ),
    Scenario(
        key="близнюки_вода",
        prompt=TWIN_WATER_PROMPT,
        system=TWINS_SYSTEM,
        schema=TWINS_SCHEMA,
        check=lambda v, ask: False,
        verdict=_twin_apart_ok,
        shown=_twin_shown,
        said=_twin_said,
        note="дзеркало: та сама видова фраза і ДВІ потреби -- дитячу воду п'є інша людина (#335)",
    ),
    Scenario(
        key="близнюки_томат_чужі",
        prompt=TWIN_FOREIGN_PROMPT,
        system=TWINS_SYSTEM,
        schema=TWINS_SCHEMA,
        check=lambda v, ask: False,
        verdict=_twin_foreign_ok,
        shown=_twin_shown,
        said=_twin_said,
        note="жива пара, яку модель не згорнула: свого артикула немає в жодного, "
        "обидва по акції, а підвид «черрі» стерла заміна",
    ),
    Scenario(
        key="близнюки_лимонад",
        prompt=TWIN_LEMONADE_PROMPT,
        system=TWINS_SYSTEM,
        schema=TWINS_SCHEMA,
        check=lambda v, ask: False,
        verdict=_twin_lemonade_ok,
        shown=_twin_shown,
        said=_twin_said,
        note="жива пара: обидва рядки свої, а під «грушу» приїхав лимон -- "
        "один смак лимонаду це одна потреба",
    ),
    Scenario(
        key="близнюки_хліб",
        prompt=TWIN_BREAD_PROMPT,
        system=TWINS_SYSTEM,
        schema=TWINS_SCHEMA,
        check=lambda v, ask: False,
        verdict=_twin_bread_ok,
        shown=_twin_shown,
        said=_twin_said,
        note="група ВІДДІЛУ: хліб, булка і багет -- різні фрази й одна потреба (24.09)",
    ),
    Scenario(
        key="близнюки_овочі",
        prompt=TWIN_VEG_PROMPT,
        system=TWINS_SYSTEM,
        schema=TWINS_SCHEMA,
        check=lambda v, ask: False,
        verdict=_twin_veg_ok,
        shown=_twin_shown,
        said=_twin_said,
        note="дзеркало: той самий відділ і три різні продукти -- не знімати жодного",
    ),
)

...
(`scripts/public_tree.py`); `types.ts` і `facts.ts` перегенеровано на цьому ж
дереві. Кожне зняття доведене: для Python -- рівність AST, для фронта -- друк
AST і компіляція компонентів, для конфігів -- рівність розібраних даних.
...
LINE_COMMENT_NAMES = frozenset({".gitignore", "pre-push", ".env.example"})
SHELL = frozenset({".sh"})
...
def docstring_lines(source: str) -> tuple[set[int], dict[int, int], int]:
    ...
    for node in ast.walk(ast.parse(source)):
        for slot in BLOCKS:
            block = getattr(node, slot, None)
            if not isinstance(block, list) or not block:
                continue
            removable: list[ast.stmt] = []
            for statement in block:
                if not _is_text(statement):
                    continue
                end_line = statement.end_lineno or statement.lineno
                before = lines[statement.lineno - 1][: statement.col_offset]
                after = lines[end_line - 1][statement.end_col_offset or 0 :]
            ...
            for statement in removable:
                ...
                taken.update(range(statement.lineno, last + 1))
            count += len(removable)
            if len(removable) == len(block) and not isinstance(node, ast.Module):
                stubs[removable[0].lineno] = removable[0].col_offset
    return taken, stubs, count
...
def strip_python(
    source: str, *, docstrings: bool = False
) -> tuple[str, int]:
    lines = source.splitlines(keepends=True)
    whole: set[int] = set()
    tails: dict[int, int] = {}
    taken, stubs, docs = (
        docstring_lines(source) if docstrings else (set(), {}, 0)
    )
...
def _undocumented(source: str) -> str:
    ...
    for node in ast.walk(tree):
        for slot in BLOCKS:
            ...
            if not isinstance(block, list) or not block:
                continue
            kept = [statement for statement in block if not _is_text(statement)]
            if not kept and not isinstance(node, ast.Module):
                kept = [ast.Expr(value=ast.Constant(value=Ellipsis))]
            setattr(node, slot, kept)
...
def env_pairs(source: str) -> list[tuple[str, str]]:
...
def verify_env(before: str, after: str) -> None:
    if env_pairs(before) != env_pairs(after):
        raise Broken("ключі або значення змінились")


def is_tsconfig(name: str) -> bool:
...
BARE_GENERATORS = ("gen_types.py", "gen_facts.py")
COMMENTS_ON = "COMMENTS = True\n"


def bare_generator(path: Path) -> None:
    text = path.read_text(encoding="utf-8")
    if text.count(COMMENTS_ON) != 1:
        raise Broken(f"{path.name}: немає рівно одного `COMMENTS = True`")
    path.write_text(
        text.replace(COMMENTS_ON, "COMMENTS = False\n"), encoding="utf-8", newline=""
    )
...
def strip_tree(tree: Path, report: Report) -> None:
    ...
    for path in sorted(tree.rglob("*")):
        ...
        if path.suffix == ".py":
            after, count = strip_python(before, docstrings=docstrings)
        ...
        elif path.suffix == ".jsonc" or is_tsconfig(path.name):
        ...
        try:
            ...
            elif is_tsconfig(path.name):
                verify_json(before, after)
...
def gates(tree: Path, report: Report, *, tests: bool, graph: bool) -> None:
    ...
    for generator in BARE_GENERATORS:
        bare_generator(tree / "scripts" / generator)
